...
#include "ft_bsd_db.h"
#include "ft_bsd_btree.h" // for BTDATAOFF/DEFMINKEYPAGE/NBLEAFDBT/indx_t
#include "ft_bsd_mpool.h" // for ft_page_hdr_t size
...
static uint32_t btree_ovfl_cutoff_bytes(uint32_t psize) {
    // Mirrors the calculation in ft_bt_open.c for t->bt_ovflsize.
...
static void run_chunk_stream(size_t total_len, const char* label) {
    ...
    TEST_ASSERT_EQUAL_INT(
        FT_OK, ft_page_policy_compute_variant_b(&bdev.geom, sizeof(ft_page_hdr_t), &pol));
    ...
    const char* cs_env = getenv("FLASH_TREE_CHUNK_SIZE");
    ...
    for (uint32_t idx = 0; idx < chunk_count; idx++) {
        ...
        expected_crc = ft_crc32_ieee(chunk_buf, n, expected_crc);
    }
    ...
    TEST_ASSERT_EQUAL_INT(FT_OK, ft_kv_put(kv, meta_key, meta_key_len, &meta, sizeof(meta)));
    ...
    for (uint32_t step = 0; step < chunk_count; step++) {
        ...
        ft_status_t put_st = ft_kv_put(kv, chunk_key, chunk_key_len, chunk_buf, n);
        if (put_st != FT_OK) {
            char msg[160];
            snprintf(msg, sizeof(msg),
                     "ft_kv_put failed: st=%d errno=%d idx=%u key_len=%zu val_len=%zu", (int)put_st,
    ...
    TEST_ASSERT_EQUAL_INT(FT_OK, ft_kv_cursor_open_prefix(kv, prefix, prefix_len, &cur));
    ...
    while (1) {
        ...
        stream_crc = ft_crc32_ieee(v.data, v.len, stream_crc);
        seen++;
    }
...
static void test_chunk_stream_1m_crc_and_order_stress(void) {
    const char* env = getenv("FLASH_TREE_STRESS");
    if (!env || env[0] == '\0' || (env[0] == '0' && env[1] == '\0')) {
        TEST_IGNORE_MESSAGE("set FLASH_TREE_STRESS=1 to enable");
    }
