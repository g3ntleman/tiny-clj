...
static void test_syskey_roundtrip(void) {
    ...
    TEST_ASSERT_EQUAL_INT(FT_OK, ft_kv_put(kv, key, sizeof(key), val, sizeof(val)));

    ft_blob_t out = {0};
    TEST_ASSERT_EQUAL_INT(FT_OK, ft_kv_get(kv, key, sizeof(key), &out));
...
static void test_syskeys_sort_before_user_keys(void) {
    ...
    TEST_ASSERT_EQUAL_INT(FT_OK, ft_kv_put(kv, sysk, sizeof(sysk), "S", 1));
    TEST_ASSERT_EQUAL_INT(FT_OK, ft_kv_put(kv, "user", 4, "U", 1));

    ft_kv_cursor_t* cur = NULL;
    TEST_ASSERT_EQUAL_INT(FT_OK, ft_kv_cursor_open_prefix(kv, NULL, 0, &cur));
...
static void test_syskey_prefix_iteration_only_returns_syskeys(void) {
    ...
    TEST_ASSERT_EQUAL_INT(FT_OK, ft_kv_put(kv, sys1, sizeof(sys1), "1", 1));
    TEST_ASSERT_EQUAL_INT(FT_OK, ft_kv_put(kv, sys2, sizeof(sys2), "2", 1));
    TEST_ASSERT_EQUAL_INT(FT_OK, ft_kv_put(kv, "user", 4, "U", 1));

    const uint8_t pfx[] = {0x00};
    ft_kv_cursor_t* cur = NULL;
    TEST_ASSERT_EQUAL_INT(FT_OK, ft_kv_cursor_open_prefix(kv, pfx, sizeof(pfx), &cur));
