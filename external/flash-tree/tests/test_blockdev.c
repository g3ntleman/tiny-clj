...
static void free_ram_bdev(ft_blockdev_t* bdev) {
    if (bdev && bdev->ctx) {
        free(bdev->ctx);
        bdev->ctx = NULL;
    }
...
static void test_erase_sets_ff(void) {
    ...
    ft_blockdev_t bdev = make_ram_bdev(storage, sizeof(storage), 1, 1, 16);
    TEST_ASSERT_EQUAL_INT(FT_OK, ft_blockdev_validate(&bdev));

    TEST_ASSERT_EQUAL_INT(FT_OK, ft_blockdev_erase(&bdev, 0, 32));
    for (int i = 0; i < 32; i++) {
...
static void test_prog_is_one_to_zero_only(void) {
    ...
    TEST_ASSERT_EQUAL_INT(FT_OK, ft_blockdev_prog(&bdev, 0, a, sizeof(a)));
    ...
    TEST_ASSERT_EQUAL_INT(FT_OK, ft_blockdev_prog(&bdev, 0, b, sizeof(b)));
    TEST_ASSERT_EQUAL_HEX8(0x0F, storage[0]);
...
static void test_granularity_enforced(void) {
    uint8_t storage[64];
    memset(storage, 0xFF, sizeof(storage));
    ft_blockdev_t bdev = make_ram_bdev(storage, sizeof(storage), 4, 4, 16);
    TEST_ASSERT_EQUAL_INT(FT_OK, ft_blockdev_validate(&bdev));

    uint8_t tmp[4] = {1, 2, 3, 4};
    TEST_ASSERT_EQUAL_INT(FT_ERR_INVALID_ARG, ft_blockdev_prog(&bdev, 2, tmp, 4));
    TEST_ASSERT_EQUAL_INT(FT_ERR_INVALID_ARG, ft_blockdev_prog(&bdev, 4, tmp, 2));
    TEST_ASSERT_EQUAL_INT(FT_ERR_INVALID_ARG, ft_blockdev_erase(&bdev, 0, 8));
    TEST_ASSERT_EQUAL_INT(FT_OK, ft_blockdev_erase(&bdev, 0, 16));
