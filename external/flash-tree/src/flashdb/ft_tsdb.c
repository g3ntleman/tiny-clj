...
#define _FDB_WRITE_STATUS(db, addr, status_table, status_num, status_index, sync)                  \
    do {                                                                                           \
        result =                                                                                   \
            _fdb_write_status((fdb_db_t)db, addr, status_table, status_num, status_index, sync);   \
...
struct log_idx_data {
    ...
    uint32_t
        log_len; /**< node total length (header + name + value), must align by FDB_WRITE_GRAN */
...
static fdb_err_t read_tsl(fdb_tsdb_t db, fdb_tsl_t tsl) {
    ...
    if ((tsl->status == FDB_TSL_PRE_WRITE) || (tsl->status == FDB_TSL_UNUSED)) {
        ...
        tsl_index_in_sector =
            (tsl->addr.index - sector_addr - SECTOR_HDR_DATA_SIZE) / LOG_IDX_DATA_SIZE;
        tsl->addr.log = sector_addr + db_sec_size(db) -
                        (tsl_index_in_sector + 1) * FDB_WG_ALIGN(FDB_TSDB_FIXED_BLOB_SIZE);
        tsl->time = idx.time;
#else
...
static uint32_t get_next_sector_addr(fdb_tsdb_t db, tsdb_sec_info_t pre_sec,
...
static uint32_t get_last_sector_addr(fdb_tsdb_t db, tsdb_sec_info_t pre_sec,
                                     uint32_t traversed_len) {
    if (traversed_len + db_sec_size(db) <= db_max_size(db)) {
...
static fdb_err_t read_sector_info(fdb_tsdb_t db, uint32_t addr, tsdb_sec_info_t sector,
    ...
    sector->status =
        (fdb_sector_store_status_t)_fdb_get_status(sec_hdr.status, FDB_SECTOR_STORE_STATUS_NUM);
    sector->start_time = sec_hdr.start_time;
    sector->end_info_stat[0] =
        (fdb_tsl_status_t)_fdb_get_status(sec_hdr.end_info[0].status, FDB_TSL_STATUS_NUM);
    sector->end_info_stat[1] =
        (fdb_tsl_status_t)_fdb_get_status(sec_hdr.end_info[1].status, FDB_TSL_STATUS_NUM);
    if (sector->end_info_stat[0] == FDB_TSL_WRITE) {
        ...
        // TODO There is no valid end node info on this sector, need impl fast query this sector by
...
static fdb_err_t update_sec_status(fdb_tsdb_t db, tsdb_sec_info_t sector, fdb_blob_t blob,
    ...
    if (sector->status == FDB_SECTOR_STORE_USING &&
        ...
        if (sector->end_info_stat[0] == FDB_TSL_UNUSED) {
            _FDB_WRITE_STATUS(db, cur_sec_addr + SECTOR_END0_STATUS_OFFSET, end_status,
                              FDB_TSL_STATUS_NUM, FDB_TSL_PRE_WRITE, false);
            ...
            FLASH_WRITE(db, cur_sec_addr + SECTOR_END0_IDX_OFFSET, &end_index, sizeof(end_index),
                        false);
            _FDB_WRITE_STATUS(db, cur_sec_addr + SECTOR_END0_STATUS_OFFSET, end_status,
                              FDB_TSL_STATUS_NUM, FDB_TSL_WRITE, true);
        } else if (sector->end_info_stat[1] == FDB_TSL_UNUSED) {
            _FDB_WRITE_STATUS(db, cur_sec_addr + SECTOR_END1_STATUS_OFFSET, end_status,
                              FDB_TSL_STATUS_NUM, FDB_TSL_PRE_WRITE, false);
    ...
    if (sector->status == FDB_SECTOR_STORE_EMPTY) {
        ...
        FLASH_WRITE(db, sector->addr + SECTOR_START_TIME_OFFSET, (uint32_t*)&cur_time,
                    sizeof(fdb_time_t), true);
    }
...
static fdb_err_t tsl_append(fdb_tsdb_t db, fdb_blob_t blob, fdb_time_t* timestamp) {
    ...
    if (blob->size > db->max_len) {
        FDB_INFO("Warning: append length (%" PRIdMAX ") is more than the db->max_len (%" PRIdMAX
 ...
 */
static int search_start_tsl_addr(fdb_tsdb_t db, int start, int end, fdb_time_t from,
                                 fdb_time_t to) {
    struct fdb_tsl tsl;
    while (true) {
 ...
 *
 * @param db database object
 * @param from starting timestamp. It will be a reverse iterator when ending timestamp less than
 * starting timestamp
...
void fdb_tsl_iter_by_time(fdb_tsdb_t db, fdb_time_t from, fdb_time_t to, fdb_tsl_cb cb,
    ...
    do {
        ...
        if ((sector.status == FDB_SECTOR_STORE_USING || sector.status == FDB_SECTOR_STORE_FULL)) {
            ...
            if ((found_start_tsl) ||
                (!found_start_tsl &&
                 ((from <= to && ((sec_addr == start_addr && from <= sector.start_time) ||
                                  from <= sector.end_time)) ||
                  (from > to && ((sec_addr == start_addr && from >= sector.end_time) ||
...
fdb_err_t fdb_tsdb_init(fdb_tsdb_t db, const char* name, const char* path, fdb_get_time get_time,
    ...
    sector.addr = 0;
    sector_iterator(db, &sector, FDB_SECTOR_STORE_UNUSED, &check_sec_arg, NULL, check_sec_hdr_cb,
                    true);
