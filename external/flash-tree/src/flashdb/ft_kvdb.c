...
#error "Please configure flash write granularity (in ft_cfg.h)"
#endif
...
static bool get_kv_from_cache(fdb_kvdb_t db, const char *name, size_t name_len, uint32_t *addr)
{
    ...
    for (i = 0; i < FDB_KV_CACHE_TABLE_SIZE; i++) {
        if ((db->kv_cache_table[i].addr != FDB_DATA_UNUSED) && (db->kv_cache_table[i].name_crc == name_crc)) {
            ...
            if (memcmp(name, saved_name, name_len) == 0) {
...
static fdb_err_t read_kv(fdb_kvdb_t db, fdb_kv_t kv)
{
    ...
    if (calc_crc32 != kv_hdr.crc32) {
        ...
        /* Flash-Tree uses binary keys: no NUL terminator required/allowed.
         * kv->name is treated as a byte array of length kv->name_len. */
    }
...
static bool find_kv_cb_ex(fdb_kv_t kv, void *arg1, void *arg2)
{
    const fdb_kv_key_ex_t *k = (const fdb_kv_key_ex_t *)arg1;
    bool *find_ok = (bool *)arg2;

    if (!k || !k->key || k->key_len == 0) return false;
    if (k->key_len != kv->name_len) return false;
    if (kv->crc_is_ok && kv->status == FDB_KV_WRITE && memcmp(kv->name, k->key, k->key_len) == 0) {
        *find_ok = true;
        return true;
    }
    return false;
...
static bool find_kv_no_cache_ex(fdb_kvdb_t db, const void *key, size_t key_len, fdb_kv_t kv)
{
    bool find_ok = false;
    fdb_kv_key_ex_t k = { key, key_len };

    kv_iterator(db, kv, (void *)&k, &find_ok, find_kv_cb_ex);
    return find_ok;
}

static bool find_kv_ex(fdb_kvdb_t db, const void *key, size_t key_len, fdb_kv_t kv)
{
    bool find_ok = false;

#ifdef FDB_KV_USING_CACHE
    if (get_kv_from_cache(db, (const char *)key, key_len, &kv->addr.start)) {
        read_kv(db, kv);
        return true;
    }
#endif /* FDB_KV_USING_CACHE */

    find_ok = find_kv_no_cache_ex(db, key, key_len, kv);

#ifdef FDB_KV_USING_CACHE
    if (find_ok) {
        update_kv_cache(db, (const char *)key, key_len, kv->addr.start);
    }
#endif /* FDB_KV_USING_CACHE */
...
static size_t get_kv_ex(fdb_kvdb_t db, const void *key, size_t key_len,
                        void *value_buf, size_t buf_len, size_t *value_len)
{
    struct fdb_kv kv;
    size_t read_len = 0;

    if (find_kv_ex(db, key, key_len, &kv)) {
        if (value_len) *value_len = kv.value_len;
        read_len = (buf_len > kv.value_len) ? kv.value_len : buf_len;
        if (value_buf) {
...
fdb_kv_t fdb_kv_get_obj_ex(fdb_kvdb_t db, const void *key, size_t key_len, fdb_kv_t kv)
{
    ...
    if (key_len > FDB_KV_NAME_MAX) {
        FDB_INFO("Error: The KV name is too long.\n");
        return NULL;
    }

    db_lock(db);
    find_ok = find_kv_ex(db, key, key_len, kv);
    db_unlock(db);
...
size_t fdb_kv_get_blob_ex(fdb_kvdb_t db, const void *key, size_t key_len, fdb_blob_t blob)
{
    ...
    db_lock(db);
    read_len = get_kv_ex(db, key, key_len, blob->buf, blob->size, &blob->saved.len);
    db_unlock(db);
    return read_len;
}
...
static fdb_err_t del_kv_ex(fdb_kvdb_t db, const void *key, size_t key_len, fdb_kv_t old_kv, bool complete_del)
{
    fdb_err_t result = FDB_NO_ERR;
    ...
    if (!old_kv) {
        ...
        if (find_kv_ex(db, key, key_len, &kv)) {
            ...
            FDB_DEBUG("Not found KV in flash.\n");
    ...
    if (!complete_del) {
        ...
        if (!db->last_is_complete_del && result == FDB_NO_ERR) {
            ...
            if (key != NULL) {
                ...
                update_kv_cache(db, (const char *)key, key_len, FDB_DATA_UNUSED);
            } else if (old_kv != NULL) {
...
static fdb_err_t move_kv(fdb_kvdb_t db, fdb_kv_t kv)
{
    ...
    if ((kv_addr = alloc_kv(db, &sector, kv->len)) != FAILED_ADDR) {
        if (db->in_recovery_check && kv->status == FDB_KV_PRE_DELETE) {
            ...
            if (find_kv_no_cache_ex(db, kv->name, kv->name_len, &kv_bak)) {
...
{
    if (!db) return FDB_INIT_FAILED;
    if (!db_init_ok(db)) return FDB_INIT_FAILED;
...
static fdb_err_t create_kv_blob_ex(fdb_kvdb_t db, kv_sec_info_t sector,
                                   ...
                                   const void *value, size_t len)
{
    ...
    kv_hdr.name_len = (uint8_t)key_len;
    ...
    if (kv_addr != FAILED_ADDR || (kv_addr = new_kv(db, sector, kv_hdr.len)) != FAILED_ADDR) {
        ...
        if (result == FDB_NO_ERR) {
            ...
            update_kv_cache(db, (const char *)key, kv_hdr.name_len, kv_addr);
#endif /* FDB_KV_USING_CACHE */
        }
        /* write value */
        if (result == FDB_NO_ERR) {
...
fdb_err_t fdb_kv_del_ex(fdb_kvdb_t db, const void *key, size_t key_len)
{
    ...
    db_lock(db);
    result = del_kv_ex(db, key, key_len, NULL, true);
    db_unlock(db);

    return result;
}

static fdb_err_t set_kv_ex(fdb_kvdb_t db, const void *key, size_t key_len, const void *value_buf, size_t buf_len)
{
    fdb_err_t result = FDB_NO_ERR;
    ...
    if (value_buf == NULL) {
        ...
        if (new_kv_ex(db, &db->cur_sector, key_len, buf_len) == FAILED_ADDR) {
            return FDB_SAVED_FULL;
        }
        kv_is_found = find_kv_ex(db, key, key_len, &db->cur_kv);
        /* prepare to delete the old KV */
        if (kv_is_found) {
            result = del_kv_ex(db, key, key_len, &db->cur_kv, false);
        }
        /* create the new KV */
        if (result == FDB_NO_ERR) {
            result = create_kv_blob_ex(db, &db->cur_sector, key, key_len, value_buf, buf_len);
        }
        /* delete the old KV */
        if (kv_is_found && result == FDB_NO_ERR) {
            result = del_kv_ex(db, key, key_len, &db->cur_kv, true);
        }
        /* process the GC after set KV */
        if (db->gc_request) {
            gc_collect_by_free_size(db, KV_HDR_DATA_SIZE + FDB_WG_ALIGN(key_len) + FDB_WG_ALIGN(buf_len));
        }
    }
...
fdb_err_t fdb_kv_set_blob_ex(fdb_kvdb_t db, const void *key, size_t key_len, fdb_blob_t blob)
{
    ...
    db_lock(db);
    result = set_kv_ex(db, key, key_len, blob->buf, blob->size);
    db_unlock(db);
...
fdb_err_t fdb_kv_set_default(fdb_kvdb_t db)
{
    ...
    for (i = 0; i < db->default_kvs.num; i++) {
        ...
        sector.empty_kv = FAILED_ADDR;
        result = create_kv_blob_ex(db, &sector,
                                   db->default_kvs.kvs[i].key,
                                   db->default_kvs.kvs[i].key_len,
                                   db->default_kvs.kvs[i].value,
                                   value_len);
        if (result != FDB_NO_ERR) {
            goto __exit;
        }
...
static void kv_auto_update(fdb_kvdb_t db)
{
    ...
    const uint8_t* ver_key = (const uint8_t*)VER_NUM_KV_NAME;
    const size_t ver_key_len = sizeof(VER_NUM_KV_NAME) - 1;

    if (get_kv_ex(db, ver_key, ver_key_len, &saved_ver_num, sizeof(size_t), NULL) > 0) {
        ...
        if (saved_ver_num != setting_ver_num) {
            ...
            for (i = 0; i < db->default_kvs.num; i++) {
                ...
                if (!find_kv_ex(db, db->default_kvs.kvs[i].key, db->default_kvs.kvs[i].key_len, &db->cur_kv)) {
                    value_len = db->default_kvs.kvs[i].value_len;
                    db->cur_sector.empty_kv = FAILED_ADDR;
                    create_kv_blob_ex(db, &db->cur_sector,
                                      db->default_kvs.kvs[i].key,
                                      db->default_kvs.kvs[i].key_len,
                                      db->default_kvs.kvs[i].value,
                                      value_len);
    ...
    (void)set_kv_ex(db, ver_key, ver_key_len, &setting_ver_num, sizeof(size_t));
}
