...
static void test_put_get_overwrite_del(void) {
    ...
    tdb_blob_t out = {0};
    TEST_ASSERT_EQUAL_INT(TDB_ERR_NOT_FOUND, tdb_kv_get(kv, "k", 1, &out));

    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_put(kv, "k", 1, "v1", 2));
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_get(kv, "k", 1, &out));
    TEST_ASSERT_EQUAL_UINT32(2, (uint32_t)out.len);
    TEST_ASSERT_EQUAL_MEMORY("v1", out.data, 2);

    // overwrite
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_put(kv, "k", 1, "v2", 2));
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_get(kv, "k", 1, &out));
    TEST_ASSERT_EQUAL_MEMORY("v2", out.data, 2);

    // delete
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_del(kv, "k", 1));
    TEST_ASSERT_EQUAL_INT(TDB_ERR_NOT_FOUND, tdb_kv_get(kv, "k", 1, &out));
...
static void test_cursor_snapshot_is_stable(void) {
    ...
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_put(kv, "a", 1, "1", 1));
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_put(kv, "aa", 2, "2", 1));
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_put(kv, "ab", 2, "3", 1));

    tdb_kv_cursor_t* cur = NULL;
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_cursor_open_prefix(kv, "a", 1, &cur));
    TEST_ASSERT_NOT_NULL(cur);

    // Mutate after opening cursor; snapshot must not change.
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_put(kv, "aa0", 3, "X", 1));
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_del(kv, "ab", 2));
...
static void test_two_cursors_are_independent_snapshots(void) {
    ...
    tdb_kv_cursor_t* a = NULL;
    tdb_kv_cursor_t* b = NULL;
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_cursor_open_prefix(kv, "a", 1, &a));
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_cursor_open_prefix(kv, "a", 1, &b));
    ...
    int has_a = 0, has_b = 0;
    tdb_blob_t ka = {0}, kb = {0};

    // Advance A once, then B twice, then A to completion (interleaved).
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_cursor_next(a, &has_a));
    TEST_ASSERT_TRUE(has_a);
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_cursor_key(a, &ka));
    TEST_ASSERT_EQUAL_UINT32(1, (uint32_t)ka.len);
    TEST_ASSERT_EQUAL_MEMORY("a", ka.data, 1);

    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_cursor_next(b, &has_b));
    TEST_ASSERT_TRUE(has_b);
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_cursor_key(b, &kb));
    ...
    tdb_kv_cursor_close(a);
    tdb_kv_cursor_close(b);
    tdb_kv_close(kv);
}
