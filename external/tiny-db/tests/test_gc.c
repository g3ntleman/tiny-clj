...
static void test_gc_allows_open_cursor_and_preserves_snapshot(void) {
    ...
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_gc_step(kv, 0));
...
static void test_recovery_persists_across_reinit(void) {
    ...
    tdb_kv_t* kv = NULL;
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_open(&kv, &bdev, NULL));
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_put(kv, "k", 1, "v", 1));
    tdb_kv_close(kv);
...
static void test_gc_persists_periodically_during_gc_steps(void) {
    ...
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_blob_put(kv, "B", 1, blob, blob_len));
    free(blob);

    /* Drive incremental GC so it takes many calls. */
    int saw_more_work = 0;
    for (int i = 0; i < 150; i++) {
        int rc = tdb_kv_gc_step_more(kv, 1);
    ...
    tdb_blob_t out = {0};
    tdb_status_t st = tdb_kv_get(kv, sys_gc_cursor, sizeof(sys_gc_cursor), &out);
    TEST_ASSERT_EQUAL_INT(TDB_OK, st);
