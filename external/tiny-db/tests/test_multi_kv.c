...
static void test_two_kv_dbs_on_different_pages_are_independent(void) {
    ...
    tdb_kv_cfg_t cfg0 = {.start_page = TDB_KV_ROOT_PAGE};
    tdb_kv_cfg_t cfg1 = {.start_page = 16}; // 16 * 4096 = 65536

    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_open(&kv0, &bdev, &cfg0));
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_open(&kv1, &bdev, &cfg1));

    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_put(kv0, "k", 1, "v0", 2));
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_put(kv1, "k", 1, "v1", 2));

    tdb_blob_t out = {0};
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_get(kv0, "k", 1, &out));
    TEST_ASSERT_EQUAL_MEMORY("v0", out.data, 2);
    TEST_ASSERT_EQUAL_INT(TDB_OK, tdb_kv_get(kv1, "k", 1, &out));
