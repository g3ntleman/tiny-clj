// tdb_crc32.c - CRC32 (IEEE 802.3 polynomial), table-based (fast).

#include "tdb_crc32.h"

/*
 * Reflected CRC32 (poly 0x04C11DB7 -> 0xEDB88320 in reflected form).
 * Semantics:
 * - crc = ~seed; process bytes; return ~crc;
 * - This supports streaming: pass the previous return value as the next seed.
 */
static const uint32_t tdb_crc32_table[256] = {
...
uint32_t tdb_crc32_ieee(const void* data, size_t len, uint32_t seed) {
    ...
    uint32_t crc = ~seed;
    while (len--) {
        crc = tdb_crc32_table[(crc ^ *p++) & 0xFFu] ^ (crc >> 8);
    }
    return ~crc;
}
