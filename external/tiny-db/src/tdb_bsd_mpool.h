...
/* Configuration */
#ifndef TDB_MPOOL_DEFAULT_PAGE_SIZE
#define TDB_MPOOL_DEFAULT_PAGE_SIZE 512 /* Used if caller passes pagesize==0 */
#endif
 ...
 */
#ifndef TDB_MPOOL_CACHE_PAGES_DEFAULT
#define TDB_MPOOL_CACHE_PAGES_DEFAULT 3
#endif

#ifndef TDB_MPOOL_CACHE_PAGES_MAX
#define TDB_MPOOL_CACHE_PAGES_MAX 32
#endif
 ...
 */
#define TDB_MPOOL_O1_RAM 1

/* Fixed cache size (only used when TDB_MPOOL_O1_RAM=1). */
#ifndef TDB_MPOOL_PG_CACHE_ENTRIES
#define TDB_MPOOL_PG_CACHE_ENTRIES 32
#endif

struct tdb_kv;

/* On-flash page header (prepended to each page in log) */
typedef struct __attribute__((packed)) tdb_page_hdr {
    uint32_t magic; /* TDB_PAGE_MAGIC */
    uint32_t pgno;  /* Logical page number */
    uint32_t crc32; /* CRC of header + page data */
    uint32_t flags; /* TDB_PAGE_FLAG_* */
} tdb_page_hdr_t;

#define TDB_PAGE_MAGIC 0x50475446u /* 'FTGP' */
/* Tombstone record: pgno is considered freed (page_offsets entry invalid). */
#define TDB_PAGE_FLAG_TOMBSTONE 0x00000001u

/* Invalid page number sentinel (page 0 is the valid B-Tree meta page!) */
#define PGNO_INVALID 0xFFFFFFFF

/* Cached page buffer */
typedef struct tdb_cache_slot {
...
#if TDB_MPOOL_O1_RAM
typedef struct tdb_pg_cache_entry {
    pgno_t pgno;
    uint32_t log_offset; /* TDB_OFF_INVALID means empty */
} tdb_pg_cache_entry_t;
#endif
...
typedef struct MPOOL {
    ...
    tdb_pg_cache_entry_t pg_cache[TDB_MPOOL_PG_CACHE_ENTRIES];
    uint32_t pg_cache_rr; /* round-robin eviction index */
#endif

    /* RAM cache */
    uint32_t cache_pagecount;
    tdb_cache_slot_t cache[TDB_MPOOL_CACHE_PAGES_MAX];
 ...
 *
 * Note: This is tiny-db specific and not part of the BSD mpool API.
 */
