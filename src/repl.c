...
ReplFormState repl_form_state(const char *source, int *error_pos) {
  ...
  if (balance > 0)
    return REPL_FORM_INCOMPLETE;
  if (balance < 0)
    return REPL_FORM_INVALID;
  return REPL_FORM_BALANCED;
}

bool repl_acc_append_line(char *acc, size_t acc_cap, const char *line) {
  if (!acc || !line || acc_cap == 0u)
    return false;

  size_t acc_len = 0u;
  while (acc_len < acc_cap && acc[acc_len] != '\0')
    acc_len++;
  if (acc_len >= acc_cap)
    return false;
  ...
  if (acc_len + extra + line_len + 1u > acc_cap)
    return false;
