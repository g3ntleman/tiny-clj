...
static inline bool map_like_contains(ID obj, ID key) {
  ...
  CljType tag = TAG(obj);
  if (tag == CLJ_MAP_PERSISTENT || tag == CLJ_MAP_TRANSIENT)
    return map_contains(obj, key) != 0;
  if (tag == CLJ_RECORD)
    return record_contains(obj, key);
  return false;
}
...
static inline int map_like_count(ID obj) {
  ...
  CljType tag = TAG(obj);
  if (tag == CLJ_MAP_PERSISTENT || tag == CLJ_MAP_TRANSIENT)
    return map_count(obj);
  if (tag == CLJ_RECORD)
    return record_count(obj);
  return 0;
}
...
static inline ID map_like_assoc(ID obj, ID key, ID value) {
  ...
  CljType tag = TAG(obj);
  if (tag == CLJ_MAP_PERSISTENT || tag == CLJ_MAP_TRANSIENT)
    return map_assoc(obj, key, value);
  if (tag == CLJ_RECORD)
    return record_assoc(obj, key, value);
  return NULL;
}
...
ID native_conj(ID *args, unsigned int argc) {
  ...
  if (is_list_type(tag)) {
    CljList *borrowed_tail = as_list(coll);
    CljList *result = borrowed_tail;
    for (unsigned int i = 1; i < argc; i++) {
      CljList *old_result = result;
      result = make_list(args[i], result);
      if (old_result != borrowed_tail) {
        RELEASE(old_result);
      }
    }
    return AUTORELEASE(result);
...
static ID unwrap_thunk_state(ID state_id) {
  if (!state_id || TAG(state_id) != CLJ_LIST)
    return state_id;
...
static CljSymbol *destructure_keyword_from_value(ID value) {
  ...
  if (len > SYMBOL_NAME_MAX_LEN)
    len = SYMBOL_NAME_MAX_LEN;
  kw_buf[0] = ':';
...
static void destructure_map(CljPersistentVector **bvec_slot, ID bform, ID init) {
  ...
  VECTOR_FOR_EACH(keys_vec, sym) {
    if (!sym)
      continue;
    if (TAG(sym) != CLJ_SYMBOL && TAG(sym) != CLJ_STRING)
      continue;
...
static ID native_map_thunk_executor(ID *args, unsigned int argc) {
  ...
  if (rest_lazy && vector_count(rest_env_stack) > 0)
    rest_lazy->thunk_state = RETAIN(vector_nth(rest_env_stack, 0));
  RELEASE(rest_thunk);
  RELEASE(rest_env_stack);
...
ID native_map(ID *args, unsigned int argc) {
  ...
  if (lazy && vector_count(env_stack) > 0)
    lazy->thunk_state = RETAIN(vector_nth(env_stack, 0));
  RELEASE(thunk);
  RELEASE(env_stack);
  return lazy ? AUTORELEASE(lazy) : empty_list();
}
...
static bool parse_timer_fn_or_opts(ID arg,
  ...
  if (out_period_ms)
    *out_period_ms = 0;
  if (out_period_from_opts)
    *out_period_from_opts = false;
  if (!arg)
    return false;
  ...
  if (period_obj != NOT_FOUND) {
    ...
    if (out_period_ms)
      *out_period_ms = period_ms;
    if (out_period_from_opts)
      *out_period_from_opts = true;
  }
...
ID native_repeat(ID *args, unsigned int argc) {
  ...
  if (argc == 1) {
    // (repeat x) - use large count as approximation of infinite sequence
    // Large but practical value (1 million) for most use cases
    count = 1000000;
    value = args[0];
  } else if (argc == 2) {
...
ID native_keyword(ID *args, unsigned int argc) {
  ...
  if (argc == 2) {
    ...
    if (TAG(args[0]) == CLJ_STRING)
      ns_str = string_data(args[0]);
    else if (TAG(args[0]) == CLJ_SYMBOL) {
      CljSymbol *s = as_symbol(args[0]);
      ns_str = s ? s->cname : NULL;
    }
    if (TAG(args[1]) == CLJ_STRING)
      name_str = string_data(args[1]);
    else if (TAG(args[1]) == CLJ_SYMBOL) {
      CljSymbol *s = as_symbol(args[1]);
      name_str = s ? s->cname : NULL;
    }
    if (!ns_str || !*ns_str || !name_str || !*name_str)
      return NULL;
    while (*ns_str == ':')
      ns_str++;
    if (!*ns_str)
  ...
  // Optional :hardware map (flat: model, cores, revision, gpio-pin-count, psram-bytes, wifi, ble, bt, ...)
  {
    ...
    if (hw.valid) {
      ...
      if (hm) {
        ...
        if (SYM_KW_CORES)
          MAP_REASSIGN(hm, map_assoc(hm, SYM_KW_CORES, fixnum((int32_t)hw.cores)));
        if (SYM_KW_REVISION)
          MAP_REASSIGN(hm, map_assoc(hm, SYM_KW_REVISION, fixnum((int32_t)hw.revision)));
        ...
        if (hw.features != 0) {
          if (SYM_KW_WIFI)
            MAP_REASSIGN(hm, map_assoc(hm, SYM_KW_WIFI, (hw.features & PLATFORM_HW_FEATURE_WIFI_BGN) ? clj_true : clj_false));
          if (SYM_KW_BLE)
            MAP_REASSIGN(hm, map_assoc(hm, SYM_KW_BLE, (hw.features & PLATFORM_HW_FEATURE_BLE) ? clj_true : clj_false));
          if (SYM_KW_BT)
            MAP_REASSIGN(hm, map_assoc(hm, SYM_KW_BT, (hw.features & PLATFORM_HW_FEATURE_BT) ? clj_true : clj_false));
          if (SYM_KW_EMB_FLASH)
            MAP_REASSIGN(hm, map_assoc(hm, SYM_KW_EMB_FLASH, (hw.features & PLATFORM_HW_FEATURE_EMB_FLASH) ? clj_true : clj_false));
          if (SYM_KW_EMB_PSRAM)
            MAP_REASSIGN(hm, map_assoc(hm, SYM_KW_EMB_PSRAM, (hw.features & PLATFORM_HW_FEATURE_EMB_PSRAM) ? clj_true : clj_false));
          if (SYM_KW_IEEE802154)
            MAP_REASSIGN(hm, map_assoc(hm, SYM_KW_IEEE802154, (hw.features & PLATFORM_HW_FEATURE_IEEE802154) ? clj_true : clj_false));
        }
        if (SYM_KW_HARDWARE)
          MAP_REASSIGN(m, map_assoc(m, SYM_KW_HARDWARE, hm));
        RELEASE(hm);
      }
    }
  }
...
static void pprint_walk(ID v, int indent, struct pprint_ctx *ctx) {
  ...
  if (tag == CLJ_MAP_PERSISTENT) {
    ...
    MAP_FOR_EACH(m, k, val) {
      for (int i = 0; i < indent + PPRINT_INDENT; i++)
        pprint_append(ctx, " ");
  ...
  if (tag == CLJ_VECTOR_PERSISTENT || tag == CLJ_VECTOR_TRANSIENT) {
    ...
    for (unsigned int i = 0; i < cnt; i++) {
      ...
      for (int j = 0; j < indent + PPRINT_INDENT; j++)
        pprint_append(ctx, " ");
