...
static inline unsigned int env_stack_count(CljVector *stack) {
    return vector_count(stack);
}

static inline CljMap *env_stack_top(CljVector *stack) {
...
static inline CljMap *env_stack_get_from_top(CljVector *stack, unsigned int depth) {
    unsigned int cnt = vector_count(stack);
...
static inline void env_stack_push_inplace(CljVector **stack_slot, CljMap *env) {
    if (!stack_slot) return;
    if (!*stack_slot) {
        // Start with a small capacity to avoid immediate growth copies.
        *stack_slot = make_vector(4, CLJ_VECTOR);
    }
    vector_conj_inplace(stack_slot, env);
}

static inline void env_stack_pop_inplace(CljVector **stack_slot) {
