...
static void builtins_fx_sound_init_symbols(void) {
#ifdef DEBUG
...
ID native_sound_host_status(ID *args, unsigned int argc) {
    ...
    builtins_fx_sound_init_symbols();
    ensure_sound_engine_initialized();
...
bool builtins_fx_sound_namespace_allowed(const char *cname, size_t ns_len) {
 ...
 */
void builtins_fx_sound_register(BuiltinsSoundRegisterFn registrar) {
    if (!registrar) {
        return;
    }
    builtins_fx_sound_init_symbols();
