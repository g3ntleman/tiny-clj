...
/**
 * @brief Computes serialized EDN length for a vector of strings without allocating.
 *
 * Mirrors the wire format used by @ref repl_history_serialize_string_vector,
 * e.g. `["cmd1" "cmd2"]`. Returns false when any element is not a string.
 *
 * @param history Candidate history vector.
 * @param out_len Receives serialized byte length (without trailing NUL).
 * @return true when @p history is a serializable string vector.
 */
static bool repl_history_string_vector_serialized_length(CljPersistentVector *history, size_t *out_len) {
  if (!history || !out_len) {
    return false;
  }
...
static CljPersistentVector *repl_history_trim_to_byte_limit(CljPersistentVector *vec,
  ...
  while (keep_count > 0) {
    ...
    if (have_repr_len && repr_len <= byte_limit) {
      return candidate;
    }
    RELEASE(candidate);
...
static bool repl_history_verify_payload(const ReplHistoryBackend *backend,
  ...
  if (stored_len == 0) {
    uint8_t dummy = 0;
    size_t loaded_len = 0;
    return backend->read(backend->ctx, &dummy, 0, &loaded_len) && loaded_len == 0;
  }
...
static bool repl_history_serialize_string_vector(CljPersistentVector *history,
  ...
  *out_len = 0;
  ...
  int count = vector_count(history);
  size_t total_len = 0;
  if (!repl_history_string_vector_serialized_length(history, &total_len)) {
    return false;
  }
