...
CljObject* make_result_channel(void) {
    ...
    // make_map returns RC=1, so map_assoc_cow will mutate in-place
    ...
    (void)map_assoc_cow((CljValue)m, (CljValue)kw_value, NULL);
    (void)map_assoc_cow((CljValue)m, (CljValue)kw_closed, (CljValue)clj_false);
    return (CljObject*)m;
}
...
void result_channel_put(CljObject *chan, CljObject *value) {
    ...
    (void)map_assoc_cow((CljValue)chan, (CljValue)kw_value, (CljValue)value);
}
...
void result_channel_close(CljObject *chan) {
    ...
    CljObject *kw_closed = intern_symbol(NULL, ":closed");
    // Channel should always be RC=1 (freshly created), so this will mutate in-place
    (void)map_assoc_cow((CljValue)chan, (CljValue)kw_closed, (CljValue)clj_true);
}
