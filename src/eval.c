...
ID eval_body_with_env(ID body, CljMap *env, EvalState *st)
{
    ...
    case CLJ_AST_NODE:
    {
        ...
        if (!body_obj || !list_type_matches(TAG(body_obj)))
            return NULL;
...
ID eval_body_with_params(ID body, const EvalContext *ctx)
{
    ...
    if (body && TAG(body) == CLJ_SYMBOL)
    {
        ...
        if (!ctx_state_for_dynamic)
            ctx_state_for_dynamic = builtin_get_eval_state();
...
ID eval_body(ID body, CljMap *env, EvalState *st, const EvalContext *ctx)
{
    ...
    case CLJ_MAP:
    {
        ...
        MAP_FOR_EACH(map, key, value)
        {
            ...
            if (eval_key && eval_key != key)
                RELEASE(eval_key);
            if (eval_value && eval_value != value)
                RELEASE(eval_value);
        }
...
ID eval_list(CljList *list, CljMap *env, EvalState *st, const EvalContext *ctx)
{
    ...
    if (original_op_sym == SYM_DEF)
        return eval_def(list, effective_env, effective_st);
    if (original_op_sym == SYM_NS)
        return eval_ns(list, effective_env, effective_st);
    ...
    if (original_op_sym == SYM_FIRST)
    {
        seq_native = native_first;
        seq_max_args = 1;
    }
    else if (original_op_sym == SYM_REST)
    {
        seq_native = native_rest;
        seq_max_args = 1;
    }
    else if (original_op_sym == SYM_CONS)
    {
        seq_native = native_cons;
        seq_max_args = 2;
    }
    else if (original_op_sym == SYM_SEQ)
    {
        seq_native = native_seq;
        seq_max_args = 1;
    }
    else if (original_op_sym == SYM_NEXT)
    {
        seq_native = native_next;
        seq_max_args = 1;
    }
    else if (original_op_sym == SYM_COUNT)
    {
        seq_native = native_count;
        seq_max_args = 1;
    }
    if (seq_native)
    {
...
ID eval_ns(CljList *list, CljMap *env, EvalState *st)
{
    ...
    for (int i = 2; i < list_len; i++)
    {
        ...
        if (!clause || !list_type_matches(TAG(clause)))
            continue;
...
ID eval_dotimes(CljList *list, CljMap *env, EvalState *st, const EvalContext *ctx)
{
    ...
    {
        CljList *binding_data = as_list(binding_list);
        if (!binding_data || !binding_data->first)
            return NULL;
...
ID eval_time(CljList *list, CljMap *env, EvalState *st, const EvalContext *ctx)
{
    ...
    if (!validate_arity(argc - 1, 1, "time"))
    { // -1 because list includes the 'time' symbol
        return NULL;
    }
