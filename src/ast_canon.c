...
static CljPersistentVector* transform_params(EvalState *st, CljPersistentVector *params, CljPersistentVector **out_let_bindings) {
    ...
    CljPersistentVector *new_params = make_vector(count, CLJ_VECTOR_PERSISTENT);
    CljPersistentVector *let_bindings = make_vector(count * 2, CLJ_VECTOR_PERSISTENT);
...
static ID canonicalize_expr_with_scope(ID expr, EvalState *st, bool in_quote, CljPersistentVector **scope_stack) {
    ...
    if (is_list_type(tag)) {
            ...
            if (head_sym == SYM_LOOP) {
                ...
                if (rest1 && rest1->first && TAG(rest1->first) == CLJ_VECTOR_PERSISTENT) {
                    ...
                    if (bindings_need_destructuring(bindings)) {
                        ...
                        CljPersistentVector *loop_bindings = make_vector(count, CLJ_VECTOR_PERSISTENT);
                        CljPersistentVector *let_bindings = make_vector(count, CLJ_VECTOR_PERSISTENT);
        ...
        if (!in_quote && first == SYM_LET && scope_stack) {
            ...
            if (rest1 && rest1->first && TAG(rest1->first) == CLJ_VECTOR_PERSISTENT) {
                ...
                CljPersistentVector *let_scope = make_vector((int)pair_count, CLJ_VECTOR_PERSISTENT);
        ...
        if (!in_quote && first == SYM_LOOP && scope_stack) {
            ...
            if (rest1 && rest1->first && TAG(rest1->first) == CLJ_VECTOR_PERSISTENT) {
                ...
                unsigned int bc = bindings_vec ? vector_count(bindings_vec) : 0;
                unsigned int pair_count = (bc / 2);
                CljPersistentVector *loop_scope = make_vector((int)pair_count, CLJ_VECTOR_PERSISTENT);
        ...
        if (!in_quote && first == SYM_DOTIMES && scope_stack) {
            ...
            if (rest1 && rest1->first && TAG(rest1->first) == CLJ_VECTOR_PERSISTENT) {
                ...
                CljPersistentVector *dotimes_scope = make_vector(1, CLJ_VECTOR_PERSISTENT);
            ...
            if (rest1) {
                ...
                if (params_form && TAG(params_form) == CLJ_VECTOR_PERSISTENT) {
                    ...
                    if (ok && !variadic) {
                        ...
                        CljPersistentVector *param_scope = make_vector((int)pc, CLJ_VECTOR_PERSISTENT);
                        VECTOR_FOR_EACH(params_vec, p) {
    ...
    if (tag == CLJ_VECTOR_PERSISTENT) {
        ...
        CljPersistentVector *new_vec = make_vector(count, CLJ_VECTOR_PERSISTENT);
