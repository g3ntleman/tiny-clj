...
static INLINE bool should_lookup_macro_for_head_symbol(CljSymbol *head_sym) {
  if (!head_sym) {
    return false;
  }
  bool is_qualified = head_sym->ns_name != NULL;
  return !is_special_symbol(head_sym) && !(is_native_symbol(head_sym) && !is_qualified);
}
...
static CljList *canonicalize_rest_to_plain_list(ID rest_expr, EvalState *st, bool in_quote, CljPersistentVector **scope_stack) {
  if (!rest_expr)
    return NULL;
  if (!is_list_type(TAG(rest_expr)))
    return NULL;
...
static bool macro_expansion_seq_needs_fallback(ID expanded_first, bool in_quote, EvalState *st) {
  if (in_quote)
    return true;
  if (!expanded_first || IS_IMMEDIATE(expanded_first))
    return true;
  if (TAG(expanded_first) != CLJ_SYMBOL)
    return false;

  CljSymbol *head_sym = symbol_normalize_core_dispatch_symbol(as_symbol(expanded_first));
  if (!head_sym)
    return true;
  if (is_special_symbol(head_sym))
    return true;
...
static bool vector_needs_destructuring(CljPersistentVector *vec, bool pairs_only) {
  ...
  VECTOR_FOR_EACH(vec, elem) {
    if ((!pairs_only || (i++ & 1) == 0) && TAG(elem) != CLJ_SYMBOL)
      return true;
  }
  return false;
}
...
static inline unsigned int scope_stack_count(CljPersistentVector *stack) {
  return vector_count(stack);
}

static inline CljPersistentVector *scope_stack_get_from_top(CljPersistentVector *stack, unsigned int depth) {
  unsigned int cnt = vector_count(stack);
  if (cnt == 0 || depth >= cnt)
    return NULL;
  return (CljPersistentVector *)vector_nth(stack, (cnt - 1) - depth);
}

static inline void scope_stack_push_inplace(CljPersistentVector **stack_slot, CljPersistentVector *scope_vec) {
  if (!stack_slot)
    return;
  if (!*stack_slot) {
    *stack_slot = make_vector(4, STRONG);
  }
  vector_conj_inplace(stack_slot, scope_vec);
}

static inline void scope_stack_pop_inplace(CljPersistentVector **stack_slot) {
  if (!stack_slot || !*stack_slot)
    return;
  if (vector_count(*stack_slot) == 0)
    return;
  vector_pop_inplace(stack_slot);
}

static bool lexical_lookup(CljPersistentVector *scope_stack, CljSymbol *sym, uint8_t *out_depth, uint8_t *out_slot) {
  if (!scope_stack || !sym || !out_depth || !out_slot)
    return false;
...
static CljPersistentVector *destructure(EvalState *st, CljPersistentVector *bindings) {
  if (!destructure_fn) {
    ...
    if (resolved == NOT_FOUND || !resolved)
      return NULL; // Bootstrap: not loaded yet
...
static CljPersistentVector *transform_params(EvalState *st, CljPersistentVector *params, CljPersistentVector **out_let_bindings) {
  ...
  CljPersistentVector *new_params = make_vector(count, STRONG);
  CljPersistentVector *let_bindings = make_vector(count * 2, STRONG);
...
static CljSymbol *canonicalize_symbol_token(CljSymbolToken *token, EvalState *st) {
  ...
  if (slash && slash > name && slash[1] != '\0') {
    ...
    if (!ns_name_sym && ns_buf[0] != '\0') {
      // Fall back to literal namespace symbol (e.g. clojure.core)
...
static ID canonicalize_expr_with_scope(ID expr, EvalState *st, bool in_quote, CljPersistentVector **scope_stack) {
  ...
  if (is_list_type(tag)) {
    ...
    CljSymbol *first_sym = (first && TAG(first) == CLJ_SYMBOL) ? as_symbol(first) : NULL;
    CljSymbol *dispatch_head_sym = symbol_normalize_core_dispatch_symbol(first_sym);
    ID dispatch_head = (ID)dispatch_head_sym;

    // Check if we're entering a quote-like expression
    // Treat (quote ...) and (quasiquote ...) as data contexts for canonicalization.
    bool is_quote_form = (dispatch_head == (ID)SYM_QUOTE || dispatch_head == (ID)SYM_QUASIQUOTE);
    bool child_in_quote = in_quote || is_quote_form;
    if (!in_quote && first_sym && dispatch_head_sym && dispatch_head_sym != first_sym) {
      first = dispatch_head;
    }
    ...
    if (!in_quote && dispatch_head_sym) {
      CljSymbol *head_sym = dispatch_head_sym;
      CljFunction *macro = NULL;
      if (should_lookup_macro_for_head_symbol(head_sym)) {
    ...
    if (!in_quote && dispatch_head == (ID)SYM_LET && scope_stack) {
      CljList *rest1 = list->rest ? as_list(list->rest) : NULL;
      if (rest1 && rest1->first && TAG(rest1->first) == CLJ_VECTOR_PERSISTENT) {
        ...
        CljPersistentVector *bindings_vec = as_vector(canon_bindings);
        unsigned int bc = bindings_vec ? vector_count(bindings_vec) : 0;
        unsigned int pair_count = (bc / 2);
        CljPersistentVector *let_scope = make_vector((int)pair_count, STRONG);
    ...
    if (!in_quote && dispatch_head == (ID)SYM_LOOP && scope_stack) {
      CljList *rest1 = list->rest ? as_list(list->rest) : NULL;
      if (rest1 && rest1->first && TAG(rest1->first) == CLJ_VECTOR_PERSISTENT) {
        // Canonicalize bindings vector without lexical rewrite.
        ID canon_bindings = canonicalize_expr_with_scope(rest1->first, st, child_in_quote, NULL);
        ...
        unsigned int bc = bindings_vec ? vector_count(bindings_vec) : 0;
        unsigned int pair_count = (bc / 2);
        CljPersistentVector *loop_scope = make_vector((int)pair_count, STRONG);
    ...
    if (!in_quote && dispatch_head == (ID)SYM_DOTIMES && scope_stack) {
      CljList *rest1 = list->rest ? as_list(list->rest) : NULL;
      if (rest1 && rest1->first && TAG(rest1->first) == CLJ_VECTOR_PERSISTENT) {
        ID canon_binding_vec = canonicalize_expr_with_scope(rest1->first, st, child_in_quote, NULL);
        CljPersistentVector *binding_vec = as_vector(canon_binding_vec);
        ...
        CljPersistentVector *dotimes_scope = make_vector(1, STRONG);
    ...
    if (!in_quote && dispatch_head == (ID)SYM_FN) {
      CljList *rest1 = list->rest ? as_list(list->rest) : NULL;
      if (rest1) {
        ...
        if (params_form && TAG(params_form) == CLJ_VECTOR_PERSISTENT) {
          ...
          CljPersistentVector *params_vec = as_vector(params_canon_id);
          ...
          if (ok) {
            VECTOR_FOR_EACH(params_vec, p) {
              ...
              if (!p || TAG(p) != CLJ_SYMBOL || IS_KEYWORD(p)) {
                ok = false;
                break;
              }
            }
          ...
          if (ok && !variadic) {
            ...
            CljPersistentVector *param_scope = make_vector((int)pc, STRONG);
            VECTOR_FOR_EACH(params_vec, p) {
...
ID ast_canon_macroexpand_1(EvalState *st, ID form) {
  ...
  CljSymbol *dispatch_head_sym = symbol_normalize_core_dispatch_symbol(head_sym);
  if (!should_lookup_macro_for_head_symbol(dispatch_head_sym)) {
    if (converted_head)
      RELEASE(converted_head);
    RETAIN(form);
    return form;
  }
  CljFunction *macro = lookup_macro_resolve(st, dispatch_head_sym);
  if (converted_head)
    RELEASE(converted_head);
  if (!macro) {
    RETAIN(form);
