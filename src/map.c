...
// Empty-map singleton: CLJ_MAP object with rc=0 and no backing store (data=NULL)
...
CljObject* map_get(CljObject *map, CljObject *key) {
    if (!map || map->type != CLJ_MAP || !key) return NULL;
...
void map_assoc(CljObject *map, CljObject *key, CljObject *value) {
    if (!map || map->type != CLJ_MAP || !key) return;
    CljMap *map_data = as_map(map);
    if (!map_data) return;
    for (int i = 0; i < map_data->count; i++) {
    ...
    if (map_data->count >= map_data->capacity) {
        ...
        CljObject **new_data = (CljObject**)calloc((size_t)new_capacity * 2, sizeof(CljObject*));
        if (!new_data) return;
        for (int i = 0; i < map_data->count; i++) {
...
CljObject* map_keys(CljObject *map) {
    if (!map || map->type != CLJ_MAP) return NULL;
    CljMap *map_data = as_map(map);
    if (!map_data) return NULL;
...
int map_count(CljObject *map) {
    if (!map || map->type != CLJ_MAP) return 0;
    CljMap *map_data = as_map(map);
...
void map_put(CljObject *map, CljObject *key, CljObject *value) {
    ...
    map_data->data = (CljObject**)realloc(map_data->data, sizeof(CljObject*) * newcap * 2);
...
void map_foreach(CljObject *map, void (*func)(CljObject*, CljObject*)) {
    if (!map || map->type != CLJ_MAP || !func) return;
    CljMap *map_data = as_map(map);
    if (!map_data) return;
...
int map_contains(CljObject *map, CljObject *key) {
    if (!map || map->type != CLJ_MAP || !key) return 0;
    CljMap *map_data = as_map(map);
...
void map_remove(CljObject *map, CljObject *key) {
    ...
    if (index >= 0) {
        ...
        if (old_key) release(old_key);
        if (old_value) release(old_value);
        for (int j = index; j < map_data->count - 1; j++) {
            KV_SET_PAIR(map_data->data, j, 
                       KV_KEY(map_data->data, j + 1),
