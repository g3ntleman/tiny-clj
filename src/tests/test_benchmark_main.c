 ...
 *
 * Consolidates all benchmark tests into a single executable for better IDE
 * integration and easier debugging while maintaining test isolation.
 */
...
void test_benchmark_collections(void) {
  ...
  for (int i = 0; i < _iterations; i++) {
    ...
    // Avoid release due to known vector finalizer issue in deep free path
    // during benchmarks
  }
  BENCHMARK_ITERATIONS_END();
}
...
static TestEntry benchmark_tests[] = {
    ...
    {"test_benchmark_simple_arithmetic", "simple",
     test_benchmark_simple_arithmetic},
    {"test_benchmark_simple_collections", "simple",
     test_benchmark_simple_collections},
    {"test_benchmark_simple_functions", "simple",
     test_benchmark_simple_functions},
    ...
    {"test_executable_size_optimization", "size",
     test_executable_size_optimization},
    {"test_executable_size_comparison", "size",
     test_executable_size_comparison},
    // clojurescript demo benchmark removed
    {"test_benchmark_repl_startup_eval", "repl", NULL},
};

static const int benchmark_test_count =
    sizeof(benchmark_tests) / sizeof(benchmark_tests[0]);
...
void list_tests(void) {
  ...
  for (int i = 0; i < benchmark_test_count; i++) {
    if (current_suite == NULL ||
        strcmp(current_suite, benchmark_tests[i].suite) != 0) {
  ...
  for (int i = 0; i < benchmark_test_count; i++) {
    if (strcmp(benchmark_tests[i].name, test_name) == 0) {
      if (benchmark_tests[i].test_func) {
        ...
        BENCHMARK_ITERATIONS("repl_startup_eval_10x", 100)
        // Simulate REPL operations without process startup overhead
        for (int r = 0; r < _iterations; r++) {
          EvalState st;
          memset(&st, 0, sizeof(EvalState));
          CljObject *expr = parse("(+ 1 2)", &st);
          (void)expr;
        }
        BENCHMARK_ITERATIONS_END();
...
int run_all_tests(void) {
  ...
  for (int i = 0; i < benchmark_test_count; i++) {
    ...
    } else if (strcmp(benchmark_tests[i].name,
                      "test_benchmark_repl_startup_eval") == 0) {
...
int compare_benchmarks(void) {
  ...
  benchmark_compare_with_previous("benchmark_report.csv",
                                  "benchmark_previous.csv");
