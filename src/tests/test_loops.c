...
TEST_SHARED(test_dotimes_zero_iterations) {
    ...
    CljObject *body = AUTORELEASE((CljObject*)make_list(SYM_PRINTLN, (CljList*)make_list(make_string("Should not print"), NULL)));
    CljObject *dotimes_call = AUTORELEASE((CljObject*)make_list(SYM_DOTIMES, (CljList*)make_list(binding_vector, (CljList*)make_list(body, NULL))));
...
TEST_SHARED(test_dotimes_simple_iteration_count) {
    ...
    CljObject *dotimes_call = AUTORELEASE((CljObject*)make_list(SYM_DOTIMES, 
