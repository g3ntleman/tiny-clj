...
void test_fixed_basic_creation(void) {
    ...
    CljValue fixed = make_fixed(1.5f);
    TEST_ASSERT_TRUE(is_fixed(fixed));
    TEST_ASSERT_FLOAT_WITHIN(0.0001f, 1.5f, as_fixed(fixed));
}

void test_fixed_negative_values(void) {
    // Test negative values
    CljValue fixed = make_fixed(-2.25f);
    TEST_ASSERT_TRUE(is_fixed(fixed));
    TEST_ASSERT_FLOAT_WITHIN(0.0001f, -2.25f, as_fixed(fixed));
}

void test_fixed_precision(void) {
    // Test precision: 0.1 + 0.2 should be close to 0.3
    CljValue a = make_fixed(0.1f);
    CljValue b = make_fixed(0.2f);
...
void test_fixed_multiplication_raw(void) {
    ...
    // 1.5 * 2.5 = 3.75
    CljValue a = make_fixed(1.5f);
    CljValue b = make_fixed(2.5f);
...
void test_fixed_saturation_max(void) {
    ...
    CljValue max = make_fixed(32767.0f);
...
void test_fixed_saturation_min(void) {
    ...
    CljValue min = make_fixed(-32768.0f);
...
void test_fixed_division_raw(void) {
    ...
    // 1.0 / 3.0 ≈ 0.333
    CljValue a = make_fixed(1.0f);
    CljValue b = make_fixed(3.0f);
    int32_t a_raw = (int32_t)((intptr_t)a >> 3);
    int32_t b_raw = (int32_t)((intptr_t)b >> 3);
...
void test_fixed_tag_consistency(void) {
    ...
    CljValue fixed = make_fixed(42.5f);
    uint8_t tag = get_tag(fixed);
