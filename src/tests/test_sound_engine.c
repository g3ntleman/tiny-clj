...
TEST(test_sound_host_catchup_advances_skipped_ticks_for_sfx_gate_timing) {
  sound_engine_init(1);

  ID track_sym = (ID)intern_symbol_global(":host-catchup-sfx-test");
  ID ba = make_test_trk1_with_delay(80, 10, 5, 1);
  TEST_ASSERT_TRUE(sound_engine_load_track(track_sym, ba));
  TEST_ASSERT_TRUE(sound_engine_play_sfx(track_sym));

  sound_engine_tick(); /* drains SFX command, NOTE parsed, END delayed */
  bool sfx_active = false;
  int sfx_slot = -1;
  for (int i = 0; i < SOUND_MAX_SFX; i++) {
    if (g_sound_engine.sfx[i].stream.active) {
  ...
  TEST_ASSERT_TRUE(sfx_active);
  TEST_ASSERT_TRUE(sfx_slot >= 0);
  TEST_ASSERT_TRUE(g_sound_engine.sfx[sfx_slot].voice_index >= 0);
  TEST_ASSERT_TRUE(g_sound_engine.voices[g_sound_engine.sfx[sfx_slot].voice_index].gate_remaining_ticks > 0u);

  /* Reproduces the host late-wakeup case from runtime logs: only 4 ticks were
   * processed while many more elapsed in wall-clock time. The host backend must
   * advance by both due and skipped ticks so the one-shot can end on time. */
  sound_backend_host_process_due_ticks(4u, 29u);

  TEST_ASSERT_FALSE(g_sound_engine.sfx[sfx_slot].stream.active);
  TEST_ASSERT_TRUE(g_sound_engine.sfx[sfx_slot].voice_index < 0);
  TEST_ASSERT_EQUAL_UINT16(0u, g_sound_engine.voices[0].freq_hz);
  TEST_ASSERT_EQUAL_UINT32(0u, g_sound_engine.voices[0].gate_remaining_ticks);
...
TEST(test_sound_host_catchup_does_not_age_new_sfx_by_skipped_backlog) {
  sound_engine_init(1);

  ID track_sym = (ID)intern_symbol_global(":host-backlog-new-sfx-test");
  ID ba = make_test_trk1_with_delay(80, 10, 5, 1);
  TEST_ASSERT_TRUE(sound_engine_load_track(track_sym, ba));
  TEST_ASSERT_TRUE(sound_engine_play_sfx(track_sym));

  /* Reproduces the runtime case from the host logs: a new SFX command arrives
   * while the scheduler reports large skipped backlog. The new command must
   * start "now", not be retroactively aged by the skipped ticks. */
  sound_backend_host_process_due_ticks(4u, 100u);
  ...
  TEST_ASSERT_TRUE(sfx_slot >= 0);
  TEST_ASSERT_TRUE(g_sound_engine.sfx[sfx_slot].voice_index >= 0);
  TEST_ASSERT_TRUE(g_sound_engine.voices[g_sound_engine.sfx[sfx_slot].voice_index].freq_hz > 0u);
  TEST_ASSERT_TRUE(g_sound_engine.voices[g_sound_engine.sfx[sfx_slot].voice_index].gate_remaining_ticks > 0u);
