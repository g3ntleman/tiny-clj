...
static ft_status_t sink_collect_cb(const uint8_t* data, size_t len, void* arg)
{
    SinkCtx* c = (SinkCtx*)arg;
    TEST_ASSERT_NOT_NULL(c);
    TEST_ASSERT_TRUE(len <= 4096); // contract: app never sees > 4KB
    if (len > c->max_seen) c->max_seen = len;
    if (c->out_len + len > c->out_cap) return FT_ERR_NO_MEMORY;
...
TEST(test_kv_stream_read_enforces_4kb_and_preserves_bytes)
{
    ...
    const uint8_t key[] = {'k'};
    TEST_ASSERT_EQUAL_INT(FT_OK, fs_kv_put_key_bytes_status(st, key, sizeof(key), data, sizeof(data)));

    TEST_ASSERT_EQUAL_INT(FT_OK, fs_stream_stats_reset(st));

    static uint8_t out[N];
    SinkCtx ctx = {.out = out, .out_cap = sizeof(out), .out_len = 0, .max_seen = 0};

    TEST_ASSERT_EQUAL_INT(FT_OK, fs_kv_stream_read_key_bytes(st, key, sizeof(key), 4096, sink_collect_cb, &ctx));
    ...
    TEST_ASSERT_EQUAL_INT(FT_OK, fs_stream_stats_get(st, &stats));
...
static ft_status_t src_var_chunks(uint8_t* out, size_t out_cap, size_t* out_len, void* arg)
{
    if (out_len) *out_len = 0;
    SrcCtx* c = (SrcCtx*)arg;
    if (!c || (!out && out_cap != 0) || !out_len) return FT_ERR_INVALID_ARG;
    if (c->pos >= c->len) return FT_OK; // EOF -> out_len stays 0
...
TEST(test_kv_stream_write_accepts_variable_input_sizes_and_roundtrips)
{
    ...
    TEST_ASSERT_EQUAL_INT(FT_OK, fs_kv_stream_write_key_bytes(st, key, sizeof(key), src_var_chunks, &src, &total_written));
    TEST_ASSERT_EQUAL_UINT32((uint32_t)sizeof(data), (uint32_t)total_written);

    static uint8_t out[N];
    size_t saved = 0;
    TEST_ASSERT_EQUAL_INT(FT_OK, fs_kv_get_key_bytes_status(st, key, sizeof(key), out, sizeof(out), &saved));
...
TEST(test_fs_file_stream_write_read_roundtrip_and_chunk_cap)
{
    ...
    SrcCtx src = {.data = data, .len = sizeof(data), .pos = 0, .call_idx = 0};
    size_t total_written = 0;
    TEST_ASSERT_EQUAL_INT(FT_OK, fs_file_stream_write(st, g_test_eval_state, "/data/s.bin", src_var_chunks, &src, &total_written));
    TEST_ASSERT_EQUAL_UINT32((uint32_t)sizeof(data), (uint32_t)total_written);
    ...
    TEST_ASSERT_EQUAL_INT(FT_OK, fs_file_stream_read(st, "/data/s.bin", 1024, sink_collect_cb, &sink));
...
TEST(test_kv_stream_read_from_offset_seeks_by_chunk_arithmetic)
{
    ...
    static uint8_t out[N];
    SinkCtx sink = {.out = out, .out_cap = sizeof(out), .out_len = 0, .max_seen = 0};
    TEST_ASSERT_EQUAL_INT(FT_OK, fs_kv_stream_read_key_bytes_from(st, key, sizeof(key), off, 4096, sink_collect_cb, &sink));
...
TEST(test_fs_file_stream_read_from_offset_seeks_by_chunk_arithmetic)
{
    ...
    SrcCtx src = {.data = data, .len = sizeof(data), .pos = 0, .call_idx = 0};
    size_t total_written = 0;
    TEST_ASSERT_EQUAL_INT(FT_OK, fs_file_stream_write(st, g_test_eval_state, "/data/seek.bin", src_var_chunks, &src, &total_written));
    TEST_ASSERT_EQUAL_UINT32((uint32_t)sizeof(data), (uint32_t)total_written);

    const size_t off = 5000;
    static uint8_t out[N];
    SinkCtx sink = {.out = out, .out_cap = sizeof(out), .out_len = 0, .max_seen = 0};
    TEST_ASSERT_EQUAL_INT(FT_OK, fs_file_stream_read_from(st, "/data/seek.bin", off, 1024, sink_collect_cb, &sink));
    TEST_ASSERT_EQUAL_UINT32((uint32_t)(N - off), (uint32_t)sink.out_len);
    TEST_ASSERT_EQUAL_MEMORY(data + off, out, N - off);
