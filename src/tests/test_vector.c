...
TEST_SHARED(test_vector_builtin_basic) {
  ...
  TEST_ASSERT_EQUAL_INT(CLJ_VECTOR_PERSISTENT, TAG(v0));
  ...
  TEST_ASSERT_EQUAL_INT(CLJ_VECTOR_PERSISTENT, TAG(v3));
...
TEST_SHARED(test_peek_and_pop_vector) {
  ...
  TEST_ASSERT_EQUAL_INT(CLJ_VECTOR_PERSISTENT, TAG(pop1));
...
TEST_SHARED(test_subvec_bounds_and_slices) {
  ...
  TEST_ASSERT_EQUAL_INT(CLJ_VECTOR_PERSISTENT, TAG(s1));
...
TEST_SHARED(test_subvec_edge_cases) {
  ...
  TEST_ASSERT_EQUAL_INT(CLJ_VECTOR_PERSISTENT, TAG(full));
  ...
  TEST_ASSERT_EQUAL_INT(CLJ_VECTOR_PERSISTENT, TAG(empty));
...
TEST_SHARED(test_vec_from_list_and_vector_id) {
  ...
  TEST_ASSERT_EQUAL_INT(CLJ_VECTOR_PERSISTENT, TAG(v));
  ...
  TEST_ASSERT_EQUAL_INT(CLJ_VECTOR_PERSISTENT, TAG(v1));
  TEST_ASSERT_EQUAL_INT(CLJ_VECTOR_PERSISTENT, TAG(v2));
...
// ============================================================================
// WEAK VECTOR TESTS (persistent weakElements=true, public API)
// ============================================================================
...
TEST_SHARED(test_weak_vector_does_not_release_on_pop) {
  WITH_AUTORELEASE_POOL({
    CljPersistentVector *weak_vec = make_weak_vector(8);
    CljPersistentMap *map = (CljPersistentMap *)make_map(4);
...
TEST_SHARED(test_weak_vector_nth_returns_same_pointer_without_retain) {
  WITH_AUTORELEASE_POOL({
    CljPersistentVector *weak_vec = make_weak_vector(8);
    CljPersistentMap *map = (CljPersistentMap *)make_map(4);
    weak_vec = vector_conj(weak_vec, (ID)map);
...
TEST_SHARED(test_clj_conj_updates_count_for_event_loop) {
  WITH_AUTORELEASE_POOL({
    ...
    TEST_ASSERT_EQUAL_INT(0, vector_count(vector_persistent(tvec)));

    // 3. Use clj_conj to add an item (like event_loop_enqueue does)
    CljPersistentMap *test_map = make_map(2);
    ...
    // 5. Add another item
    CljPersistentMap *test_map2 = make_map(2);
...
TEST_SHARED(test_clj_conj_with_empty_transient_vector) {
  WITH_AUTORELEASE_POOL({
    ...
    ID elem0 = vector_nth(vector_persistent(tvec), 0);
    TEST_ASSERT_EQUAL_PTR(test_map, elem0);
...
TEST_SHARED(test_transient_vector_assoc_keeps_pointer_and_updates_backing_store) {
  WITH_AUTORELEASE_POOL({
    ...
    vector_assoc_inplace(&tvec->backing, 1, fixnum(99));
...
TEST_SHARED(test_equal_persistent_and_transient_vector) {
  ...
  WITH_AUTORELEASE_POOL({
    ...
    TEST_ASSERT_EQUAL_INT(SPECIAL_FALSE, as_special(equal_transient));
...
TEST_SHARED(test_transient_on_transient_returns_same_object) {
  ...
  // Transients do not have value semantics
...
TEST_SHARED(test_vector_for_each_empty_vector) {
  CljPersistentVector *vec = AUTORELEASE(make_vector(0, false));
...
// NOTE: low-level transient vector_set_nth API was removed; tests above exercised
// transient mutation semantics. We keep only the persistent error test below.
