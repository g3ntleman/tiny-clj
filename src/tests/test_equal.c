...
TEST_SHARED(test_vector_equal_same_vectors) {
  WITH_MEMORY_PROFILING({
    ...
    ASSIGN(vec1_vec, vector_conj(vec1_vec, val1));
    ASSIGN(vec1_vec, vector_conj(vec1_vec, val2));
    ASSIGN(vec1_vec, vector_conj(vec1_vec, val3));
    vec1_val = (CljValue)vec1_vec;

    CljPersistentVector *vec2_vec = as_persistent_vector(vec2_val);
    ASSIGN(vec2_vec, vector_conj(vec2_vec, val1));
    ASSIGN(vec2_vec, vector_conj(vec2_vec, val2));
    ASSIGN(vec2_vec, vector_conj(vec2_vec, val3));
...
TEST_SHARED(test_vector_equal_different_values) {
  WITH_MEMORY_PROFILING({
    ...
    CljPersistentVector *vec1_vec = as_persistent_vector(vec1_val);
    ASSIGN(vec1_vec, vector_conj(vec1_vec, int1));
    ASSIGN(vec1_vec, vector_conj(vec1_vec, int2));
    vec1_val = (CljValue)vec1_vec;

    CljPersistentVector *vec2_vec = as_persistent_vector(vec2_val);
    ASSIGN(vec2_vec, vector_conj(vec2_vec, int3));
    ASSIGN(vec2_vec, vector_conj(vec2_vec, int4));
    vec2_val = (CljValue)vec2_vec;

    // Verify vectors were created successfully
    TEST_ASSERT_NOT_NULL((CljObject *)vec1_val);
    TEST_ASSERT_NOT_NULL((CljObject *)vec2_val);
    TEST_ASSERT_EQUAL_INT(CLJ_VECTOR_PERSISTENT, TAG(vec1_val));
    TEST_ASSERT_EQUAL_INT(CLJ_VECTOR_PERSISTENT, TAG(vec2_val));
    ...
    RELEASE((CljObject *)vec1_val);
    RELEASE((CljObject *)vec2_val);
  });
}
...
TEST_SHARED(test_vector_equal_with_strings) {
  WITH_MEMORY_PROFILING({
    ...
    CljPersistentVector *vec1_vec = as_persistent_vector(vec1_val);
    ASSIGN(vec1_vec, vector_conj(vec1_vec, (ID)str1));
    ASSIGN(vec1_vec, vector_conj(vec1_vec, (ID)str2));
    vec1_val = (CljValue)vec1_vec;

    CljPersistentVector *vec2_vec = as_persistent_vector(vec2_val);
    ASSIGN(vec2_vec, vector_conj(vec2_vec, (ID)str3));
    ASSIGN(vec2_vec, vector_conj(vec2_vec, (ID)str4));
    vec2_val = (CljValue)vec2_vec;
...
TEST_SHARED(test_map_equal_after_add_and_remove) {
  WITH_MEMORY_PROFILING({
    ...
    ASSIGN(map, map_assoc(map, key1, val1));
    ASSIGN(map, map_assoc(map, key2, val2));
