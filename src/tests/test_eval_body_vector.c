...
#define TEST_SHARED_DEFAULT_HEAP_GROWTH_LIMIT 60000
...
TEST_SHARED(test_eval_body_vector_with_env_stack) {
    ...
    CljPersistentMap *x_binding = map_assoc(map_empty(), (ID)x_sym, fixnum(1));
    CljPersistentMap *y_binding = map_assoc(map_empty(), (ID)y_sym, fixnum(2));
...
TEST_SHARED(test_eval_body_vector_with_base_env) {
    ...
    CljPersistentMap *base_env = map_assoc(map_empty(), x_sym, fixnum(1));
