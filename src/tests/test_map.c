...
TEST(test_map_get_finds_let_binding) {
  ...
  CljPersistentMap *let_env = AUTORELEASE(make_map(4));
  ...
  CljValue found = map_get_sentinel((CljPersistentMap *)let_env, (CljValue)step_sym, NULL);
  ...
  CljValue found2 = map_get_sentinel((CljPersistentMap *)let_env, (CljValue)step_sym2, NULL);
...
TEST(test_map_get_structural_comparison) {
  ...
  CljValue found = map_get_sentinel((CljPersistentMap *)let_env, (CljValue)step_sym2, NULL);
  TEST_ASSERT_NOT_NULL(found);
  TEST_ASSERT_EQUAL_INT(42, as_fixnum(found));
}
...
TEST(test_map_assoc_performance_unchanged) {
  CljPersistentMap *map = AUTORELEASE(make_map(100));
  ...
  CljValue val = (CljValue)map_get_sentinel((CljPersistentMap *)map, (CljValue)kw, NULL);
...
TEST(test_update_basic) {
  ...
  CljValue val = map_get_sentinel((CljPersistentMap *)result, (CljValue)key, NULL);
  TEST_ASSERT_NOT_NULL(val);
...
TEST(test_update_missing_key_simple) {
  ...
  CljPersistentMap *map = AUTORELEASE(make_map_from_stack(pairs, 1));
  ...
  CljValue val_b_before = map_get_sentinel((CljPersistentMap *)map, (CljValue)key_b, NULL);
  ...
  CljPersistentMap *result = map_assoc(map, (CljValue)key_b, (CljValue)new_val_b);
  ...
  CljPersistentMap *result_map = result;
  TEST_ASSERT_EQUAL_INT(2, result_map->count);

  // Step 5: Verify the new key was added
  CljValue val_b_after = map_get_sentinel((CljPersistentMap *)result, (CljValue)key_b, NULL);
  ...
  CljValue val_a = map_get_sentinel((CljPersistentMap *)result, (CljValue)key_a, NULL);
...
TEST(test_update_missing_key) {
  ...
  CljPersistentMap *result_map = as_map((CljObject *)result);
  TEST_ASSERT_EQUAL_INT(2, result_map->count);

  // Verify the new key was added
  CljSymbol *key_b = intern_symbol(NULL, ":b");
  CljValue val_b = map_get_sentinel((CljPersistentMap *)result, (CljValue)key_b, NULL);
...
TEST(test_map_assoc_direct) {
  ...
  // Verify the new key was added
  CljValue retrieved_val_b = map_get_sentinel((CljPersistentMap *)result, (CljValue)key_b, NULL);
  ...
  // Verify original key still exists
  CljSymbol *key_a = intern_symbol(NULL, ":a");
  CljValue retrieved_val_a = map_get_sentinel((CljPersistentMap *)result, (CljValue)key_a, NULL);
...
TEST(test_map_transient_comprehensive) {
  ...
  CljTransientMap *result = map_transient(NULL);
  TEST_ASSERT_NULL(result);

  // Test 2: Empty map conversion
  CljPersistentMap *empty_map = AUTORELEASE(make_map(4));
  TEST_ASSERT_EQUAL_INT(0, empty_map->count);
  CljTransientMap *empty_transient = AUTORELEASE(map_transient(empty_map));
  TEST_ASSERT_NOT_NULL(empty_transient);
  TEST_ASSERT_TRUE(TAG((CljObject *)empty_transient) == CLJ_MAP_TRANSIENT);
  TEST_ASSERT_EQUAL_INT(0, map_count(empty_transient));

  // Test 3: Map with entries - conversion and data preservation
  CljPersistentMap *persistent_map = AUTORELEASE(make_map(8));
  CljObject *keys[5];
  for (int i = 0; i < 5; i++) {
    char key_name[16];
  ...
  CljTransientMap *transient_map = map_transient(persistent_map);
  TEST_ASSERT_NOT_NULL(transient_map);
  TEST_ASSERT_TRUE(TAG((CljObject *)transient_map) == CLJ_MAP_TRANSIENT);
  TEST_ASSERT_EQUAL_INT(5, map_count(transient_map));
  ...
  for (int i = 0; i < 5; i++) {
    CljValue val_persistent = map_get_sentinel((CljPersistentMap *)persistent_map, (CljValue)keys[i], NULL);
    CljValue val_transient = map_get_sentinel((CljPersistentMap *)transient_map, (CljValue)keys[i], NULL);
  ...
  CljTransientMap *result2 = map_transient((CljPersistentMap *)transient_map);
...
TEST(test_map_conj_comprehensive) {
  ...
  map_conj(NULL, (CljValue)intern_symbol(NULL, ":key"), fixnum(42));

  CljPersistentMap *base = AUTORELEASE(make_map(4));
  CljTransientMap *transient = AUTORELEASE(map_transient(base));

  // Test: map_conj with NULL key should succeed (nil is a valid key in Clojure)
  map_conj(transient, NULL, fixnum(42));
  TEST_ASSERT_EQUAL_INT(1, map_count(transient));

  // Test 2: Add new key-value pair to transient map (already has nil key)
  CljObject *key1 = (CljObject *)intern_symbol(NULL, ":a");
  map_conj(transient, (CljValue)key1, fixnum(1));
  TEST_ASSERT_EQUAL_INT(2, map_count(transient)); // nil and :a

  CljValue val1 = map_get_sentinel(transient, (CljValue)key1, NULL);
  TEST_ASSERT_NOT_NULL(val1);
  TEST_ASSERT_TRUE(is_fixnum(val1));
  TEST_ASSERT_EQUAL_INT(1, as_fixnum(val1));

  // Test 3: Update existing key
  map_conj(transient, (CljValue)key1, fixnum(100));
  TEST_ASSERT_EQUAL_INT(2, map_count(transient)); // Count unchanged

  CljValue val1_updated = map_get_sentinel(transient, (CljValue)key1, NULL);
  ...
  TEST_ASSERT_EQUAL_INT(4, map_count(transient)); // nil, :a, :b, :c

  CljValue val2 = map_get_sentinel(transient, (CljValue)key2, NULL);
  CljValue val3 = map_get_sentinel(transient, (CljValue)key3, NULL);
  ...
  TEST_ASSERT_EQUAL_INT(5, map_count(transient));

  CljValue val4 = map_get_sentinel(transient, (CljValue)key4, NULL);
  TEST_ASSERT_NULL(val4); // NULL value is valid

  // Test 6: Capacity growth should work
  CljObject *key5 = (CljObject *)intern_symbol(NULL, ":e");
  map_conj(transient, (CljValue)key5, fixnum(5));
  TEST_ASSERT_EQUAL_INT(6, map_count(transient));
}
...
TEST(test_map_conj_with_interned_symbols_across_contexts) {
  ...
  CljPersistentMap *base = AUTORELEASE(make_map(4));
  CljTransientMap *tmap = AUTORELEASE(map_transient(base));
  ...
  CljValue closed_val1 = map_get_sentinel((CljPersistentMap *)tmap, (CljValue)kw_closed, NULL);
  ...
  map_conj(tmap, kw_closed_new, clj_true);

  // Verify update worked
  CljValue closed_val2 = map_get_sentinel((CljPersistentMap *)tmap, (CljValue)kw_closed, NULL);
...
TEST(test_map_conj_finds_existing_key_by_pointer) {
  ...
  map_conj(tmap, kw2, fixnum(100));

  // Verify value was updated (not added)
  CljValue val = map_get_sentinel((CljPersistentMap *)tmap, (CljValue)kw, NULL);
  TEST_ASSERT_NOT_NULL(val);
  TEST_ASSERT_TRUE(is_fixnum(val));
...
TEST(test_map_conj_channel_pattern) {
  ...
  CljPersistentMap *base = AUTORELEASE(make_map(4));
  CljTransientMap *chan = AUTORELEASE(map_transient(base));
  ...
  // Verify initial state
  CljValue closed_before = map_get_sentinel((CljPersistentMap *)chan, (CljValue)kw_closed, NULL);
  ...
  map_conj(chan, kw_closed_close, clj_true);

  // Verify channel was mutated
  CljValue closed_after = map_get_sentinel((CljPersistentMap *)chan, (CljValue)kw_closed, NULL);
  ...
  // Verify count didn't increase
  TEST_ASSERT_EQUAL_INT(2, map_count(chan));
}
...
TEST(test_dissoc_single_key) {
  WITH_AUTORELEASE_POOL({
    ...
    CljPersistentMap *result_map = (CljPersistentMap *)result;
    TEST_ASSERT_EQUAL_INT(2, result_map->count);

    // Verify :a and :c are still present
    CljValue val_a = map_get_sentinel((CljPersistentMap *)result_map, (CljValue)key_a, NULL);
    CljValue val_c = map_get_sentinel((CljPersistentMap *)result_map, (CljValue)key_c, NULL);
    ...
    // Verify :b is removed
    CljValue val_b = map_get_sentinel((CljPersistentMap *)result_map, (CljValue)key_b, NULL);
...
TEST(test_map_assoc_nil_key) {
  WITH_AUTORELEASE_POOL({
    CljPersistentMap *map = AUTORELEASE(map_empty());
...
TEST(test_map_literal_nil_key) {
  WITH_AUTORELEASE_POOL({
    ...
    CljPersistentMap *map = (CljPersistentMap *)result;
    TEST_ASSERT_EQUAL_INT(1, map->count);
...
TEST(test_map_contains_nil_key_not_exists) {
  WITH_AUTORELEASE_POOL({
    // Create map without nil key
    CljPersistentMap *map = AUTORELEASE(map_empty());
    CljObject *key = AUTORELEASE(intern_symbol(NULL, ":key"));
...
TEST(test_get_nil_key_from_map_literal) {
  WITH_AUTORELEASE_POOL({
    ...
    CljPersistentMap *map = (CljPersistentMap *)map_obj;
    TEST_ASSERT_EQUAL_INT(1, map->count);
...
TEST(test_map_get_not_found_sentinel_edge_cases) {
  WITH_AUTORELEASE_POOL({
    CljPersistentMap *map = AUTORELEASE(map_empty());
    CljSymbol *key1 = AUTORELEASE(intern_symbol_global("key1"));
    CljSymbol *key2 = AUTORELEASE(intern_symbol_global("key2"));
    CljSymbol *key3 = AUTORELEASE(intern_symbol_global("key3"));
    CljSymbol *missing_key = AUTORELEASE(intern_symbol_global("missing"));
...
#define ASSERT_TRUE_RESULT(expr)                                     \
  do {                                                               \
...
#define ASSERT_FALSE_RESULT(expr)                                      \
  do {                                                                 \
    CljObject *r = eval_string(expr, g_test_eval_state);               \
    TEST_ASSERT_NOT_NULL_MESSAGE(r, expr " should not be nil");        \
...
// H1/H2: Test AUTORELEASE map_assoc with ASSIGN pattern
TEST(test_map_assoc_autorelease_assign_hypothesis) {
...
// H1/H2/H5: Test multiple ASSIGN calls in sequence (simulates ns_define behavior)
TEST(test_map_assoc_multiple_assign_hypothesis) {
  CljPersistentMap *map = make_map(4);
  TEST_ASSERT_NOT_NULL(map);
...
TEST(test_embedded_array_memory_efficiency) {
  WITH_AUTORELEASE_POOL({
    ...
    CljPersistentMap *map1 = make_map(2);
    CljPersistentMap *map2 = make_map(4);
    CljPersistentMap *map3 = make_map(8);
    ...
    TEST_ASSERT_NOT_NULL(map_get_sentinel((CljPersistentMap *)map1, fixnum(1), NULL));
    TEST_ASSERT_NOT_NULL(map_get_sentinel((CljPersistentMap *)map2, fixnum(2), NULL));
    TEST_ASSERT_NOT_NULL(map_get_sentinel((CljPersistentMap *)map3, fixnum(3), NULL));
...
TEST(test_embedded_array_cow) {
  WITH_AUTORELEASE_POOL({
    ...
    TEST_ASSERT_NULL(map_get_sentinel((CljPersistentMap *)map, fixnum(2), NULL));
...
TEST(test_embedded_array_capacity_growth) {
  WITH_AUTORELEASE_POOL({
    CljPersistentMap *map = make_map(2); // Small capacity
...
TEST(test_embedded_array_performance) {
  WITH_AUTORELEASE_POOL({
    ...
    CljValue val25 = map_get_sentinel((CljPersistentMap *)env, fixnum(25), NULL);
