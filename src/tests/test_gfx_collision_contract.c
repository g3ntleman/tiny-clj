...
TEST(test_gfx_scene_rgb_channel_color_conversion) {
    ...
    ID out = eval_string(
        ...
        "  (require 'tiny-fx.gfx) "
        "  (require 'tiny-fx.gfx-collision) "
        "  [(tiny-fx.gfx/color 0 255 255) "
        "   (tiny-fx.gfx/color 255 255 255) "
        "   (tiny-fx.gfx/color 255 0 255) "
        "   (tiny-fx.gfx/color 255 0 0)])",
        g_test_eval_state);
    TEST_ASSERT_NOT_NULL(out);
    TEST_ASSERT_TRUE(TAG(out) == CLJ_VECTOR_PERSISTENT);
    CljPersistentVector *v = as_vector(out);
...
static void assert_tinyfx_color_illegal_argument(const char *expr) {
    TEST_ASSERT_NOT_NULL(g_test_eval_state);
    ...
    TEST_ASSERT_TRUE_MESSAGE(exception_caught, "Expected IllegalArgumentException from tiny-fx.gfx/color");
}

TEST(test_gfx_scene_rgb888_int_color_invalid_input) {
    assert_tinyfx_color_illegal_argument(
        "(do "
        "  (require 'tiny-fx.gfx) "
        "  (require 'tiny-fx.gfx-scene) "
        "  (require 'tiny-fx.gfx) "
...
TEST(test_gfx_scene_rgb_channel_color_invalid_input) {
    assert_tinyfx_color_illegal_argument(
        ...
        "  (require 'tiny-fx.gfx) "
        "  (require 'tiny-fx.gfx-collision) "
        "  (tiny-fx.gfx/color nil 0 0))");
    assert_tinyfx_color_illegal_argument(
        "(do "
        "  (require 'tiny-fx.gfx) "
        "  (tiny-fx.gfx/color -1 0 0))");
    assert_tinyfx_color_illegal_argument(
        "(do "
        "  (require 'tiny-fx.gfx) "
        "  (tiny-fx.gfx/color 256 0 0))");
    assert_tinyfx_color_illegal_argument(
        "(do "
        "  (require 'tiny-fx.gfx) "
        "  (tiny-fx.gfx/color 0 3.14 0))");
}
