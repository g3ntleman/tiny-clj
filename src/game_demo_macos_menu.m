...
static id g_macos_fx_performance_activity = nil;
static bool g_macos_fx_window_autosave_bound = false;
static bool g_macos_fx_window_callbacks_registered = false;
static bool g_macos_fx_screen_restore_applied = false;
static NSTimer *g_macos_fx_runloop_watchdog_timer = nil;
static NSView *g_macos_fx_runloop_watchdog_overlay = nil;
static NSTextField *g_macos_fx_runloop_watchdog_label = nil;
static bool g_macos_fx_runloop_watchdog_stalled = false;
...
static void macos_fx_set_performance_activity(id token) {
    id old_token = g_macos_fx_performance_activity;
    if (old_token == token) {
        return;
    }
    [token retain];
    [old_token release];
    g_macos_fx_performance_activity = token;
}

static NSNumber *macos_fx_screen_number(NSScreen *screen) {
    if (!screen) {
        return nil;
    }
...
static NSString *macos_fx_screen_uuid_string(NSScreen *screen) {
    NSNumber *screen_number = macos_fx_screen_number(screen);
    if (!screen_number) {
        return nil;
    }
...
static NSScreen *macos_fx_find_screen_by_uuid(NSString *target_uuid) {
    if (!target_uuid || [target_uuid length] == 0) {
        return nil;
    }
    NSArray<NSScreen *> *screens = [NSScreen screens];
    for (NSScreen *screen in screens) {
        NSString *uuid = macos_fx_screen_uuid_string(screen);
...
static uint64_t macos_fx_monotonic_now_ns(void) {
    struct timespec ts;
    (void)clock_gettime(CLOCK_MONOTONIC, &ts);
    return ((uint64_t)ts.tv_sec * 1000000000ull) + (uint64_t)ts.tv_nsec;
}

static void macos_fx_store_screen_position_for_window(NSWindow *window) {
    if (!window) {
        return;
    }
    ...
    NSString *screen_uuid = macos_fx_screen_uuid_string(screen);
    if (!screen_uuid) {
        return;
    }
...
static void macos_fx_apply_saved_screen_placement(NSWindow *window) {
    if (!window || g_macos_fx_screen_restore_applied) {
        return;
    }
    ...
    if (!saved_screen_uuid || [saved_screen_uuid length] == 0) {
        g_macos_fx_screen_restore_applied = true;
        return;
    }
    NSScreen *target_screen = macos_fx_find_screen_by_uuid(saved_screen_uuid);
    if (!target_screen) {
...
static void macos_fx_bind_window_autosave(NSWindow *window) {
    if (!window || g_macos_fx_window_autosave_bound) {
        return;
    }
    macos_fx_apply_saved_screen_placement(window);
    [window setFrameAutosaveName:kMacosViewerWindowAutosaveName];
    g_macos_fx_window_autosave_bound = true;
}

static NSWindow *macos_fx_primary_window(void) {
...
static void macos_fx_hide_runloop_watchdog_overlay(void) {
    if (g_macos_fx_runloop_watchdog_overlay) {
        [g_macos_fx_runloop_watchdog_overlay setHidden:YES];
    }
}

static void macos_fx_position_runloop_watchdog_overlay(NSWindow *window) {
    if (!window || !g_macos_fx_runloop_watchdog_overlay) {
        return;
    }
    ...
    [g_macos_fx_runloop_watchdog_overlay setFrame:frame];
}

static void macos_fx_ensure_runloop_watchdog_overlay(NSWindow *window) {
    if (!window) {
        return;
    }
    NSView *content_view = [window contentView];
    ...
    if (!g_macos_fx_runloop_watchdog_overlay) {
        g_macos_fx_runloop_watchdog_overlay =
            [[NSView alloc] initWithFrame:NSMakeRect(0, 0, kMacosViewerRunloopWatchdogWidth, kMacosViewerRunloopWatchdogHeight)];
        [g_macos_fx_runloop_watchdog_overlay setWantsLayer:YES];
        [[g_macos_fx_runloop_watchdog_overlay layer] setCornerRadius:8.0];
        [[g_macos_fx_runloop_watchdog_overlay layer]
            setBackgroundColor:[[NSColor colorWithCalibratedRed:0.74 green:0.14 blue:0.14 alpha:0.92] CGColor]];
        [g_macos_fx_runloop_watchdog_overlay setAutoresizingMask:NSViewMaxXMargin | NSViewMinYMargin];

        g_macos_fx_runloop_watchdog_label =
            [[NSTextField alloc] initWithFrame:NSMakeRect(10.0, 10.0, kMacosViewerRunloopWatchdogWidth - 20.0, 32.0)];
        [g_macos_fx_runloop_watchdog_label setEditable:NO];
        [g_macos_fx_runloop_watchdog_label setSelectable:NO];
        [g_macos_fx_runloop_watchdog_label setBezeled:NO];
        [g_macos_fx_runloop_watchdog_label setBordered:NO];
        [g_macos_fx_runloop_watchdog_label setDrawsBackground:NO];
        [g_macos_fx_runloop_watchdog_label setTextColor:[NSColor whiteColor]];
        [g_macos_fx_runloop_watchdog_label setFont:[NSFont boldSystemFontOfSize:12.0]];
        [g_macos_fx_runloop_watchdog_label setLineBreakMode:NSLineBreakByWordWrapping];
        [g_macos_fx_runloop_watchdog_label setStringValue:@""];
        [g_macos_fx_runloop_watchdog_overlay addSubview:g_macos_fx_runloop_watchdog_label];
        [g_macos_fx_runloop_watchdog_overlay setHidden:YES];
    }
    if ([g_macos_fx_runloop_watchdog_overlay superview] != content_view) {
        [g_macos_fx_runloop_watchdog_overlay removeFromSuperview];
        [content_view addSubview:g_macos_fx_runloop_watchdog_overlay];
    }
    macos_fx_position_runloop_watchdog_overlay(window);
}

static void macos_fx_update_runloop_watchdog_overlay(ViewerRunloopLivenessSnapshot snapshot) {
    if (snapshot.state != FX_RUNLOOP_LIVENESS_STALLED) {
        ...
        g_macos_fx_runloop_watchdog_stalled = false;
        macos_fx_hide_runloop_watchdog_overlay();
        return;
    }

    NSWindow *window = macos_fx_primary_window();
    if (window) {
        macos_fx_ensure_runloop_watchdog_overlay(window);
        NSString *message =
            [NSString stringWithFormat:@"Runloop stalled\nLast tick %.1fs ago, iterations %llu",
                                       (double)snapshot.age_ns / 1e9,
                                       (unsigned long long)snapshot.iteration_count];
        [g_macos_fx_runloop_watchdog_label setStringValue:message];
        [g_macos_fx_runloop_watchdog_overlay setHidden:NO];
    }
    if (!g_macos_fx_runloop_watchdog_stalled) {
        fprintf(stderr,
    ...
    g_macos_fx_runloop_watchdog_stalled = true;
}
...
- (void)runloopWatchdogTimerFired:(NSTimer *)timer {
    (void)timer;
    ViewerRunloopLivenessSnapshot snapshot = fx_runloop_liveness_snapshot(macos_fx_monotonic_now_ns());
    macos_fx_update_runloop_watchdog_overlay(snapshot);
}
@end

static MacosViewerRunloopWatchdogObserver *g_macos_fx_runloop_watchdog_observer = nil;
...
- (void)windowDidBecomeMainOrKey:(NSNotification *)note {
    ...
    macos_fx_bind_window_autosave(window);
}
...
- (void)windowDidMoveOrChangeScreen:(NSNotification *)note {
    ...
    if (!g_macos_fx_screen_restore_applied) {
        return;
    }
    NSWindow *window = (NSWindow *)obj;
    macos_fx_store_screen_position_for_window(window);
}
@end

static MacosViewerWindowObserver *g_macos_fx_window_observer = nil;
...
void macos_fx_register_window_callbacks(void) {
    @autoreleasepool {
        if (g_macos_fx_window_callbacks_registered) {
            return;
        }
        if (!g_macos_fx_window_observer) {
            g_macos_fx_window_observer = [MacosViewerWindowObserver new];
        }
        NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
        [nc addObserver:g_macos_fx_window_observer
        ...
        [nc addObserver:g_macos_fx_window_observer
               selector:@selector(windowDidMoveOrChangeScreen:)
                 ...
                 object:nil];
        g_macos_fx_window_callbacks_registered = true;
    }
}

void macos_fx_start_runloop_watchdog(void) {
    @autoreleasepool {
        if (g_macos_fx_runloop_watchdog_timer) {
            return;
        }
        if (!g_macos_fx_runloop_watchdog_observer) {
            g_macos_fx_runloop_watchdog_observer = [MacosViewerRunloopWatchdogObserver new];
        }
        g_macos_fx_runloop_watchdog_stalled = false;
        macos_fx_hide_runloop_watchdog_overlay();
        g_macos_fx_runloop_watchdog_timer =
            [[NSTimer scheduledTimerWithTimeInterval:kMacosViewerRunloopWatchdogPollIntervalSeconds
                                              target:g_macos_fx_runloop_watchdog_observer
...
void macos_fx_stop_runloop_watchdog(void) {
    @autoreleasepool {
        if (g_macos_fx_runloop_watchdog_timer) {
            [g_macos_fx_runloop_watchdog_timer invalidate];
            [g_macos_fx_runloop_watchdog_timer release];
            g_macos_fx_runloop_watchdog_timer = nil;
        }
        g_macos_fx_runloop_watchdog_stalled = false;
        macos_fx_hide_runloop_watchdog_overlay();
    }
...
void macos_fx_begin_performance_activity(void) {
    @autoreleasepool {
        if (g_macos_fx_performance_activity != nil) {
            return;
        }
        ...
        macos_fx_set_performance_activity(token);
        [token release];
    }
}
...
void macos_fx_end_performance_activity(void) {
    @autoreleasepool {
        ...
        id token = g_macos_fx_performance_activity;
        [token retain];
        macos_fx_set_performance_activity(nil);
        NSProcessInfo *proc = [NSProcessInfo processInfo];
