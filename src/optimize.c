...
static bool symbols_equal(CljSymbol *sym1, CljSymbol *sym2) {
    ...
    CLJ_ASSERT(sym1->name && sym2->name);
...
bool is_tail_position(CljObject *expr, CljObject *body) {
    if (!expr || !body || !is_type(body, CLJ_LIST)) return false;
...
void validate_recur_positions(CljObject *body, CljObject *parent_body) {
    if (!body || !is_type(body, CLJ_LIST)) return;
    CljList *body_list = as_list((ID)body);
    if (!body_list) return;
...
CljObject* transform_recursive_tail_calls(CljObject *body, CljObject *func_name, 
    ...
    if (head == SYM_LET) {
        ...
        CljList *new_let = build_list((CljObject*)SYM_LET, t_bindings ? t_bindings : bindings, (CljObject*)t_body);
        if (!new_let) {
            if (t_bindings && t_bindings != bindings) RELEASE(t_bindings);
            RELEASE((CljObject*)t_body);
            return NULL;
        }
    ...
    if (t_rest) {
        ...
        if (new_body) {
            new_body->rest = (CljObject*)t_rest;
