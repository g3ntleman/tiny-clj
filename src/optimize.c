...
static CljRecordDescriptor *record_descriptor_from_ctor_call(ID target_expr) {
  if (!target_expr || TAG(target_expr) != CLJ_AST_CALL)
    return NULL;
...
static bool slot_frame_resolve_target(const RecordSlotFrame *slot_env, CljSlotRef *slot_ref, ID *out_target) {
  ...
  if (!frame)
    return false;
  if ((unsigned int)slot_ref->slot >= frame->slot_count)
    return false;
...
static bool rewrite_record_lookup_call(CljASTCall *call,
  ...
  if (IS_KEYWORD(call->op)) {
    ...
    if (!symbol_cname_equals(op_sym, "get"))
      return false;
    if (argc != 2 && argc != 3)
      return false;
    target_expr = vector_nth(call->args, 0);
...
bool is_tail_position(CljObject *expr, CljObject *body) {
  ...
  if (TAG(body) == CLJ_AST_CALL) {
    ...
    if (head == SYM_IF) {
      if (argc >= 2) {
        ...
        if (then_expr == expr || is_tail_position(expr, then_expr))
          return true;
      }
      if (argc >= 3) {
        ID else_expr = vector_nth(args, 2);
        if (else_expr == expr || is_tail_position(expr, else_expr))
          return true;
      }
      return false;
  ...
  if (head == SYM_IF) {
    ...
    if (then_list) {
      if (then_list->first == expr)
        return true;
      if (is_tail_position(expr, then_list->first))
        return true;
      CljList *else_list = as_list(then_list->rest);
      if (else_list && (else_list->first == expr || is_tail_position(expr, else_list->first)))
        return true;
    }
  } else if (head == SYM_WHEN) {
    // (when cond body...) - last body is tail
    CljList *body_exprs = as_list(rest->rest);
    if (body_exprs && is_last_in_list(expr, body_exprs))
      return true;
  } else if (head == SYM_DO) {
    // (do expr...) - last expr is tail
    if (is_last_in_list(expr, rest))
      return true;
  } else if (head == SYM_LET) {
    // (let [bindings] body...) - last body is tail
    CljList *body_exprs = as_list(rest->rest);
    if (body_exprs) {
      if (is_last_in_list(expr, body_exprs))
        return true;
      if (last_is_tail_position(expr, body_exprs))
        return true;
    }
  } else if (head == SYM_COND) {
    ...
    bool is_test = true;
    while (rest) {
      if (!is_test && (rest->first == expr || is_tail_position(expr, rest->first)))
        return true;
...
bool is_recursive_call(CljObject *call_expr, CljObject *func_name) {
  ...
  if (!called_name || TAG(called_name) != CLJ_SYMBOL)
    return false;
...
static bool expr_contains_recur(ID expr) {
  ...
  if (TAG(expr) == CLJ_AST_CALL) {
    ...
    if (!call)
      return false;
    if (call->op == (ID)SYM_RECUR)
      return true;
    if (expr_contains_recur(call->op))
      return true;
    if (call->args) {
      unsigned int argc = vector_count(call->args);
  ...
  if (TAG(expr) == CLJ_VECTOR_PERSISTENT || TAG(expr) == CLJ_VECTOR_TRANSIENT) {
    ...
    for (unsigned int i = 0; i < count; i++) {
      if (expr_contains_recur(vector_nth(vec, i)))
        return true;
    }
    return false;
  }

  if (!is_list_type(TAG(expr)))
    return false;
  CljList *list = as_list(expr);
  while (list) {
    if (expr_contains_recur(list->first))
      return true;
