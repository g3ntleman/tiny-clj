...
ID native_format(ID *args, unsigned int argc) {
  ...
  if (!fmt_str || TAG(args[0]) != CLJ_STRING)
    return NULL;
  ...
  if (argc == 1) {
    ...
    while (idx < fmt_len) {
      if (fmt_data[idx] == '%' && (idx + 1) < fmt_len) {
        ...
        case 'f': {
          // Float
          float val = (TAG(args[arg_idx]) == CLJ_INT) ? (float)AS_FIXNUM(args[arg_idx]) : as_fixed((CljValue)args[arg_idx]);
          int n = 0;
          while (true) {
            size_t used = (size_t)(out - buffer);
            size_t avail = buf_size - used;
