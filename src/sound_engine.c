...
static void sound_ensure_finished_event_keywords(void) {
    ...
    KW_SOUND = intern_symbol_global(":sound");
...
bool sound_engine_play_music(ID track_id, int32_t repeat) {
    ...
    if (ok && !g_sound_engine.tick_running) {
        sound_tick_start();
    }
    return ok;
}
...
static void tick_drain_commands(void) {
    ...
    while (drained < SOUND_CMD_QUEUE_CAP && lockfree_spsc_queue_pop(&g_sound_engine.cmd_queue.spsc, &cmd)) {
            ...
            for (int i = 0; i < SOUND_MAX_SFX; i++) {
                if (g_sound_engine.sfx[i].stream.active &&
                    ...
                    if (g_sound_engine.sfx[i].voice_index >= 0) {
                        g_sound_engine.voices[g_sound_engine.sfx[i].voice_index].active = false;
                    }
                }
            }
            break;
        ...
        case SOUND_CMD_PLAY_SFX: {
            ...
            g_sound_engine.sfx[slot].priority = 128; /* default priority */
            break;
        }
...
static void tick_advance_stream(SoundStream *s) {
    if (!s->active) return;
...
static void tick_advance_sfx(void) {
    for (int i = 0; i < SOUND_MAX_SFX; i++) {
        ...
        while (sfx->stream.active && sfx->stream.current_tick >= sfx->stream.next_event_tick
            ...
            events_this_tick++;
            SoundVoice temp_voice = g_sound_engine.voices[vi];
            bool had_more = stream_parse_event(&sfx->stream, &temp_voice, 1);
            g_sound_engine.voices[vi] = temp_voice;
        ...
        if (!sfx->stream.active) {
            /* Release voice */
            g_sound_engine.voices[vi].active = false;
            g_sound_engine.voices[vi].freq_hz = 0;
            sfx->voice_index = -1;
        }
    }
}
    ...
    for (int i = 0; i < g_sound_engine.voice_count; i++) {
        ...
        if (v->active && v->gate_remaining_ticks > 0) {
            ...
            if (v->gate_remaining_ticks == 0) {
                ...
                uint8_t effective_vol = (uint8_t)(((uint16_t)v->volume * g_sound_engine.music_volume) >> 8);
                sound_backend_set_voice(i, v->freq_hz, effective_vol);
            }
        } else if (!v->active) {
            sound_backend_set_voice(i, 0, 0);
        }
...
void sound_engine_tick(void) {
    tick_drain_commands();
    tick_advance_stream(&g_sound_engine.music_stream);
    tick_advance_sfx();
    tick_update_voices();
