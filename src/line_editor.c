...
struct LineEditor {
    ...
    CljPersistentVector *history;        // CljPersistentVector for history
...
LineEditor* line_editor_new(GetCharFunc get_char, PutCharFunc put_char, PutStringFunc put_string, void *ctx) {
    ...
    CljPersistentVector *persistent_vec = make_vector(50, CLJ_VECTOR_PERSISTENT);  // Start with persistent vector
    editor->history = (CljPersistentVector*)vector_transient(persistent_vec);      // Convert to transient for efficient operations
...
void line_editor_add_to_history(LineEditor *editor, const char *line) {
    ...
    if (line_obj) {
        vector_conj_inplace((CljPersistentVector**)&editor->history, line_obj);
...
void line_editor_clear_history(LineEditor *editor) {
    ...
    history_exit(editor);
    RELEASE(editor->history);
    CljPersistentVector *persistent_vec = make_vector(50, CLJ_VECTOR_PERSISTENT);
    editor->history = (CljPersistentVector*)vector_transient(persistent_vec);
...
bool line_editor_history_save_default(CljObject *vec) {
    ...
    return history_save_to_file((CljPersistentVector*)vec, path);
}
