...
# Get all test groups
GROUPS=$(./build-test/unity-tests --list 2>&1 | grep -E "^test_" | cut -d'/' -f1 | sort -u)
...
for group in $GROUPS; do
    ...
    timeout 5 ./build-test/unity-tests "$group/*" 2>&1 | tail -20
    ...
    if [ $? -eq 124 ]; then
        ...
        TESTS=$(./build-test/unity-tests --list 2>&1 | grep "^$group/")
        
        for test in $TESTS; do
            echo "  Testing: $test"
            timeout 3 ./build-test/unity-tests "$test" 2>&1 | tail -5
            if [ $? -eq 124 ]; then
                echo "    ❌ TEST $test HUNG"
            fi
        done
