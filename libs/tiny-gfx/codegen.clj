...
;; tiny-gfx.codegen
;;
;; Consolidated code generation helpers for tiny-gfx:
;; - C header generation from record schema
;; - SVG -> draw-record conversion

;; ---------------------------------------------------------------------------
;; C header generation from tiny-gfx.scene record layouts
;; ---------------------------------------------------------------------------

(def record-layouts
  [{:type-name "Transform" :prefix "TINY_GFX_TRANSFORM" :fields scene/transform-fields}
   {:type-name "Style" :prefix "TINY_GFX_STYLE" :fields scene/style-fields}
   {:type-name "Group" :prefix "TINY_GFX_GROUP" :fields scene/group-fields}
   {:type-name "Line" :prefix "TINY_GFX_LINE" :fields scene/line-fields}
   {:type-name "Polyline" :prefix "TINY_GFX_POLYLINE" :fields scene/polyline-fields}
   {:type-name "Rect" :prefix "TINY_GFX_RECT" :fields scene/rect-fields}
   {:type-name "Tri" :prefix "TINY_GFX_TRI" :fields scene/tri-fields}
   {:type-name "VText" :prefix "TINY_GFX_VTEXT" :fields scene/text-fields}
   {:type-name "Scene" :prefix "TINY_GFX_SCENE" :fields scene/scene-fields}
   {:type-name "FrameScene" :prefix "TINY_GFX_FRAME_SCENE" :fields scene/frame-scene-fields}])
...
(defn emit-record-enum
  ...
  (let [lines (loop [i 0
                ...
                (if (empty? fs)
                  out
                  (let [field (first fs)
                        line (str "  " prefix "_IDX_" (tiny-gfx.codegen/kw->c-token field) " = " i ",")]
...
(defn c-header-string
  "Returns generated C header content for tiny-gfx.scene record layouts."
  []
  (str "/* Auto-generated by tiny-gfx.codegen. */\n"
       "#ifndef TINY_GFX_SCENE_LAYOUT_H\n"
       "#define TINY_GFX_SCENE_LAYOUT_H\n\n"
       (str/join "\n"
                 (map tiny-gfx.codegen/emit-record-enum record-layouts))
       "\n#endif /* TINY_GFX_SCENE_LAYOUT_H */\n"))
