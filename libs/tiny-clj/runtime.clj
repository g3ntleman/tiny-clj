...
(defn start-renderer! [& args] :native)

^#^{:doc "Stops the renderer thread. Usage: (stop-renderer!).
Returns true on success, false when unsupported in the active runtime backend."}
(defn stop-renderer! [] :native)
...
(defn renderer-state [slot-id entity-id] :native)
...
(defn renderer-timeline-step [slot-id entity-id field-key] :native)
...
(defn renderer-timeline-progress [slot-id entity-id field-key] :native)
