...
/** Compile pattern; returns regex object (rc=1) or NULL with error message. */
CljRegex *regex_compile(const char *pattern, char *error, size_t error_size) {
    if (!pattern) {
        if (error && error_size > 0) {
            mini_snprintf(error, error_size, "Pattern cannot be null");
        }
        return NULL;
    }
    ...
    if (len >= MAX_PATTERN_LEN) {
        if (error && error_size > 0) {
            mini_snprintf(error, error_size, "Pattern too long (max %d chars)", MAX_PATTERN_LEN - 1);
        }
        return NULL;
    }
    ...
    if (validation_error) {
        if (error && error_size > 0) {
            mini_snprintf(error, error_size, "%s", validation_error);
        }
        return NULL;
    }
    ...
    if (!compiled) {
        if (error && error_size > 0) {
            mini_snprintf(error, error_size, "Invalid regex pattern");
        }
        return NULL;
    }
