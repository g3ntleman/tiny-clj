...
size_t tinyclj_format_inst_literal_iso8601_utc(char *buf, size_t buf_size, int32_t days, uint32_t ms) {
  ...
  return (size_t)mini_snprintf(buf, buf_size,
      "#inst \"%04u-%02u-%02u"  "T%02u:%02u:%02u.%03u" "Z\"",
      y, (unsigned)month, (unsigned)day,
      (unsigned)hour, (unsigned)minute, (unsigned)second, (unsigned)millis);
}
