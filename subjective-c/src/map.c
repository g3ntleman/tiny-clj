...
ID map_keys(CljMap *map) {
  ...
  if (!map_data)
    return NULL;
  CljPersistentVector* keys_vec = make_vector(map_data->count, CLJ_VECTOR_PERSISTENT);
  if (!keys_vec)
    return NULL;
...
ID map_vals(CljMap *map) {
  ...
  if (!map_data)
    return NULL;
  CljPersistentVector* vals_vec = make_vector(map_data->count, CLJ_VECTOR_PERSISTENT);
  if (!vals_vec)
    return NULL;
